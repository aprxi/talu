...
use talu::{
    CollabError, CollabHandle, CollabParticipantKind as CoreParticipantKind,
    CollabWatchDurability as CoreWatchDurability, CollabWatchEventType as CoreWatchEventType,
};
...
async fn collab_watch_wait_blocking(
    ...
    after_seq: u64,
    timeout_ms: u64,
) -> Result<talu::CollabWatchWaitResult, String> {
...
pub(crate) fn resolve_storage_root(
    ...
    let scope = auth
        .map(|ctx| ctx.storage_prefix.as_str())
        .filter(|value| !value.is_empty())
        .unwrap_or("default");

    // TODO: make persistent collab storage bind to TALU_DB_HOST. Until that is
    // implemented the collab core is process-local and in-memory only, so this
    // root is just a logical namespace partition.
    let db_host = std::env::var("TALU_DB_HOST").ok();
    Ok(storage_root_for_scope(scope, db_host.as_deref()))
}

fn storage_root_for_scope(scope: &str, db_host: Option<&str>) -> String {
    match db_host {
        Some(host) if !host.trim().is_empty() => format!("db://{host}/{scope}"),
        _ => format!("memory://{scope}"),
    }
}
...
mod tests {
    ...
    fn test_state(collab_enabled: bool) -> AppState {
        AppState {
            backend: Arc::new(Mutex::new(crate::server::state::BackendState {
                backend: None,
                current_model: None,
            })),
            ...
            bucket_path: None::<PathBuf>,
            workdir: None::<PathBuf>,
            collab_enabled,
            agent_policy: None,
            shell_sessions: Mutex::new(HashMap::new()),
            process_sessions: Mutex::new(HashMap::new()),
            collab_handles: Mutex::new(HashMap::new()),
            agent_runtime_mode: crate::server::AgentRuntimeMode::Host,
            sandbox_backend: crate::server::SandboxBackend::LinuxLocal,
            shell_session_ttl: Duration::from_secs(60),
            process_session_ttl: Duration::from_secs(60),
    ...
    #[test]
    fn storage_root_for_scope_defaults_to_memory_without_db_host() {
        let root = storage_root_for_scope("default", None);
        assert_eq!(root, "memory://default");
    }

    #[test]
    fn storage_root_for_scope_uses_db_host_when_present() {
        let root = storage_root_for_scope("tenants/acme", Some("db.internal:7258"));
        assert_eq!(root, "db://db.internal:7258/tenants/acme");
    }

    #[test]
    fn resolve_storage_root_rejects_disabled_collab() {
        let state = test_state(false);
        let resp = resolve_storage_root(&state, None).expect_err("disabled collab error");
