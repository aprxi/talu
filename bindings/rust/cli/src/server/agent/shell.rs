...
//! (`talu_shell_*` via `crate::shell::ShellSession`).
...
pub async fn handle_create(
    ...
    let shell = match talu::ShellSession::open_with_policy_runtime(
        cols,
        rows,
        ...
        Err(e) => {
            let (status, code) = match &e {
                talu::ShellError::PolicyDeniedCwd(_) => {
                    (StatusCode::FORBIDDEN, "policy_denied_cwd")
                }
                talu::ShellError::PolicyDeniedExec(_) | talu::ShellError::CommandDenied(_) => {
                    (StatusCode::FORBIDDEN, "policy_denied_exec")
                }
                talu::ShellError::StrictUnavailable(_) => (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "strict_runtime_unavailable",
                ),
                talu::ShellError::StrictSetupFailed(_) => (
                    StatusCode::INTERNAL_SERVER_ERROR,
...
async fn handle_ws_connection(
    state: Arc<AppState>,
    shell_id: String,
    shell: Arc<Mutex<talu::ShellSession>>,
    upgraded: Upgraded,
) -> Result<(), String> {
    let mut ws: WsStream =
