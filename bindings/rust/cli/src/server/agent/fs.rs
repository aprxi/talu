...
//!
//! HTTP handlers are glue only: parse request JSON, call `crate::FsHandle`,
//! and shape responses.
...
fn load_policy(state: &AppState) -> Option<Arc<talu::Policy>> {
...
async fn sync_workdir_collab_op(
    ...
    let _ = collab.submit_op(
        ...
        Some(issued_at_ms),
        snapshot,
        talu::CollabWatchDurability::Strong,
    );
}
...
async fn clear_workdir_collab_snapshot(
    ...
    let _ = collab.clear_snapshot(
        COLLAB_SYNC_ACTOR_ID,
        talu::CollabParticipantKind::System,
        Some("sync"),
        op_kind,
    );
}
