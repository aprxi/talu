...
//! `crate::process::ProcessSession`, and stream bytes as SSE or WebSocket.
...
pub async fn handle_spawn(
    ...
    let process = match talu::ProcessSession::open_with_policy_runtime(
        &request.command,
        ...
        Err(e) => {
            let (status, code) = match &e {
                talu::ProcessError::CommandDenied(_) => (StatusCode::FORBIDDEN, "command_denied"),
                talu::ProcessError::PolicyDeniedCwd(_) => {
                    (StatusCode::FORBIDDEN, "policy_denied_cwd")
                }
                talu::ProcessError::PolicyDeniedExec(_) => {
                    (StatusCode::FORBIDDEN, "policy_denied_exec")
                }
                talu::ProcessError::StrictUnavailable(_) => (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "strict_runtime_unavailable",
                ),
                talu::ProcessError::StrictSetupFailed(_) => (
                    StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_ws(
    ...
    let accept = super::compute_accept_key(&key);
...
async fn handle_ws_connection(
    state: Arc<AppState>,
    process_id: String,
    process: Arc<Mutex<talu::ProcessSession>>,
    upgraded: Upgraded,
) -> Result<(), String> {
    let mut ws: WsStream =
