...
pub(super) struct LsArgs {
    /// Target: Org prefix (Qwen) or Org/Model for files
...
mod tests {
    ...
    fn parse_convert_defaults() {
        let cli = parse(&["talu", "convert", "Qwen/Qwen3.5-2B", "--scheme", "mxfp8"])
            .expect("parse should succeed");
    ...
    #[test]
    fn parse_serve_rejects_workdir_flag() {
        assert!(parse(&["talu", "serve", "--workdir", "/tmp/workdir"]).is_err());
    }
