...
fn fork_creates_new_session() {
    ...
    let source = get(ctx.addr(), "/v1/chat/sessions/sess-fork-src");
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-src/fork",
        &json!({"target_item_id": 0}),
    );
...
fn fork_nonexistent_source_returns_404() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/nonexistent/fork",
...
fn fork_missing_target_item_id_returns_400() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-bad/fork",
        &json!({}),
    );
...
fn fork_with_prefix_path() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-prefix/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201, "body: {}", resp.body);
}
...
fn fork_new_session_appears_in_list() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-list/fork",
        &json!({"target_item_id": 0}),
    );
...
fn fork_new_session_is_retrievable() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-get/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);
    let new_id = resp.json()["id"].as_str().expect("id").to_string();
    ...
    let resp = get(ctx.addr(), &format!("/v1/chat/sessions/{new_id}"));
...
fn fork_preserves_items_subset() {
    ...
    let source_resp = get(ctx.addr(), "/v1/chat/sessions/sess-fork-subset");
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-subset/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);
    let new_id = resp.json()["id"].as_str().expect("id").to_string();
    ...
    let fork_resp = get(ctx.addr(), &format!("/v1/chat/sessions/{new_id}"));
...
fn fork_invalid_json_returns_400() {
    ...
    let resp = send_request(
        ctx.addr(),
        "POST",
        "/v1/chat/sessions/sess-fork-badjson/fork",
...
fn fork_string_target_item_id_returns_400() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-strid/fork",
...
fn fork_does_not_modify_source() {
    ...
    let before = get(ctx.addr(), "/v1/chat/sessions/sess-fork-src-intact");
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-src-intact/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);

    // Get source after fork — should be unchanged
    let after = get(ctx.addr(), "/v1/chat/sessions/sess-fork-src-intact");
...
fn fork_negative_target_item_id_returns_400() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-neg/fork",
        &json!({"target_item_id": -1}),
    );
    assert_eq!(resp.status, 400, "body: {}", resp.body);
}
...
fn fork_float_target_item_id_returns_400() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-float/fork",
...
fn fork_null_target_item_id_returns_400() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-null/fork",
...
fn fork_boolean_target_item_id_returns_400() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-bool/fork",
...
fn fork_extra_fields_ignored() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-extra/fork",
...
fn fork_response_has_object_field() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-obj/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);
...
fn fork_returns_201_not_200() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-201/fork",
        &json!({"target_item_id": 0}),
    );
...
fn fork_each_call_creates_unique_session() {
    ...
    let resp1 = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-unique/fork",
        &json!({"target_item_id": 0}),
    );
...
fn fork_very_large_item_id() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-large/fork",
...
fn fork_zero_target_item_id_returns_201() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-zero/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);
}
...
fn fork_inherits_model_from_source() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-model/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);
    let new_id = resp.json()["id"].as_str().expect("id").to_string();
...
fn fork_response_has_created_at_and_updated_at() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-ts/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);
    let json = resp.json();
...
fn fork_response_has_marker_field() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-marker/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);
    let json = resp.json();
...
fn fork_content_type_is_json() {
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-ct/fork",
        &json!({"target_item_id": 0}),
    );
    assert_eq!(resp.status, 201);
...
fn fork_at_mid_item_has_fewer_items_than_source() {
    ...
    // Get source items count.
    let source_resp = get(ctx.addr(), "/v1/chat/sessions/sess-fork-mid");
    assert_eq!(source_resp.status, 200);
    let source_json = source_resp.json();
    let source_items = source_json["items"].as_array().expect("source items");
    let source_count = source_items.len();
    ...
    let resp = post_json(
        ctx.addr(),
        "/v1/chat/sessions/sess-fork-mid/fork",
        &json!({"target_item_id": 1}),
    );
    assert_eq!(resp.status, 201);
    let new_id = resp.json()["id"].as_str().expect("id").to_string();
