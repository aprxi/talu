...
//!
//! Enable with `--features fixtures` and set `TALU_TOKENIZER_FIXTURES` to the
//! dataset root.
...
fn render_fixture_chat_template(
    ...
    assert!(
        !out.is_null(),
        "chat template render succeeded but output is null"
    );
...
fn fixtures_root() -> PathBuf {
    let root = std::env::var("TALU_TOKENIZER_FIXTURES").unwrap_or_else(|_| {
        panic!(
            "fixture tests require TALU_TOKENIZER_FIXTURES=<dataset_root> when --features fixtures is enabled"
        )
    });
    let path = PathBuf::from(root);
    assert!(
        path.is_dir(),
        "TALU_TOKENIZER_FIXTURES points to a non-directory path: {}",
        path.display()
    );
    path
}
...
impl ModelStats {
    ...
    fn fail(&mut self, detail: String) {
        self.failed += 1;
        self.all_examples.push(detail.clone());
...
/// Write the full (untruncated) failure details to a temp file and return its
/// path for inclusion in panic output.
fn write_full_failure_artifact(
    label: &str,
    stats: &BTreeMap<String, ModelStats>,
) -> Option<PathBuf> {
    let mut out = String::new();
    for (name, s) in stats.iter().filter(|(_, s)| s.failed > 0) {
        out.push_str(&format!("--- {name} ({} failures) ---\n", s.failed));
        for ex in &s.all_examples {
            out.push_str(ex);
            out.push('\n');
        }
        out.push('\n');
    ...
    let path = std::env::temp_dir().join(format!("talu_{label}_fixture_failures.log"));
    fs::write(&path, out).ok().map(|_| path)
}
...
fn fixture_encode() {
    ...
    if total_fail > 0 {
        if let Some(path) = write_full_failure_artifact("encode", &stats) {
            panic!("\n{report}\n\nfull failure log: {}", path.display());
        }
        panic!("\n{report}");
    }
}
...
fn fixture_encode_special() {
    ...
    if total_fail > 0 {
        if let Some(path) = write_full_failure_artifact("encode_special", &stats) {
            panic!("\n{report}\n\nfull failure log: {}", path.display());
        }
        panic!("\n{report}");
    }
...
fn fixture_roundtrip() {
    ...
    if total_fail > 0 {
        if let Some(path) = write_full_failure_artifact("roundtrip", &stats) {
            panic!("\n{report}\n\nfull failure log: {}", path.display());
        }
        panic!("\n{report}");
    }
...
fn fixture_vocab_decode() {
    ...
    if total_fail > 0 {
        if let Some(path) = write_full_failure_artifact("vocab_decode", &stats) {
            panic!("\n{report}\n\nfull failure log: {}", path.display());
        }
        panic!("\n{report}");
    }
...
fn fixture_chat_template() {
        ...
        for entry in &expected.chat_template {
            ...
            match render_fixture_chat_template(
                ...
                Ok(actual) if actual != *expected_text => {
                    s.fail(format!(
                        "chat_template({:?})\n  expected: {:?}\n  actual:   {:?}",
                        entry.name, expected_text, actual
                    ));
                }
                Err(rc) => {
                    s.fail(format!(
    ...
    if total_fail > 0 {
        if let Some(path) = write_full_failure_artifact("chat_template", &stats) {
            panic!("\n{report}\n\nfull failure log: {}", path.display());
        }
        panic!("\n{report}");
    }
