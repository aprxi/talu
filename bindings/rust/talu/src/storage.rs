...
impl StorageHandle {
    ...
    pub fn list_sessions_batch(
        ...
        let result = unsafe {
            talu_sys::talu_db_session_list_batch(
                self.path_cstr.as_ptr(),
                offset as u32,
                limit as u32,
                group_ptr,
    ...
    pub fn load_session(
        ...
        let ptr = unsafe {
            talu_sys::talu_db_session_load_conversation(
                self.path_cstr.as_ptr(),
                session_id_cstr.as_ptr(),
            )
        };
    ...
    pub fn update_session(
        ...
        let result = unsafe {
            talu_sys::talu_db_session_update_ex(
                self.path_cstr.as_ptr(),
                session_id_cstr.as_ptr(),
                title_ptr,
                marker_ptr,
    ...
    pub fn list_tags(&self, group_id: Option<&str>) -> Result<Vec<TagRecord>, StorageError> {
        ...
        // Free C memory
        unsafe { talu_sys::talu_db_tag_free_list(c_list) };
    ...
    pub fn remove_session_tag(&self, session_id: &str, tag_id: &str) -> Result<(), StorageError> {
        ...
        let result = unsafe {
            talu_sys::talu_db_session_remove_tag(
                self.path_cstr.as_ptr(),
                session_cstr.as_ptr(),
                tag_cstr.as_ptr(),
            )
    ...
    pub fn get_session_tags(&self, session_id: &str) -> Result<Vec<String>, StorageError> {
        ...
        let result = unsafe {
            talu_sys::talu_db_session_get_tags(
                self.path_cstr.as_ptr(),
                session_cstr.as_ptr(),
                &mut c_list as *mut _,
            )
        ...
        // Free C memory
        unsafe { talu_sys::talu_db_session_free_relation_list(c_list) };
    ...
    pub fn get_tag_sessions(&self, tag_id: &str) -> Result<Vec<String>, StorageError> {
        ...
        let result = unsafe {
            talu_sys::talu_db_tag_get_conversations(
                self.path_cstr.as_ptr(),
                tag_cstr.as_ptr(),
                &mut c_list as *mut _,
            )
    ...
    pub fn get_sessions_tags_batch(
        ...
        let result = unsafe {
            talu_sys::talu_db_session_get_tags_batch(
                self.path_cstr.as_ptr(),
                c_ptrs.as_ptr(),
        ...
        if !c_batch.is_null() {
            ...
            unsafe { talu_sys::talu_db_session_free_tag_batch(c_batch) };
        }
