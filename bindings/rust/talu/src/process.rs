...
#[cfg(test)]
mod tests {
    use super::ProcessSession;

    #[test]
    fn process_session_roundtrip_output_and_exit_code() {
        let mut process = ProcessSession::open("echo hello", None).unwrap();
        let mut buf = [0u8; 4096];
        let mut output = Vec::new();
        let mut guard = 0usize;

        while process.is_alive().unwrap_or(false) && guard < 50_000 {
