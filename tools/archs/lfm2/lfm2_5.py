"""
LFM2.5 - Pure PyTorch implementation.

Supports LiquidAI LFM2.5 (Hybrid Conv + Attention).

Differs from LFM2 in config format:
- LFM2 uses full_attn_idxs: [2, 5, 8, ...] to mark attention layers
- LFM2.5 uses layer_types: ["conv", "conv", "full_attention", ...] per-layer

Usage:
    from lfm2.lfm2_5 import LFM2_5

    model, tokenizer = LFM2_5.from_pretrained("LiquidAI/LFM2.5-1.2B-Thinking")
...
class ShortConv(nn.Module):
    """
    LFM2.5 Short Convolution block.
...
class Attention(nn.Module):
    """
    LFM2.5 Attention with QK LayerNorm.
...
class MLP(nn.Module):
    """
    LFM2.5 MLP with SwiGLU activation.
...
class HybridBlock(nn.Module):
    """
    LFM2.5 decoder block - either ShortConv or Attention based on layer index.

    Layer type is determined by layer_types config:
    - layer_types[layer_idx] == "full_attention": use Attention
    - layer_types[layer_idx] == "conv": use ShortConv
    """
    ...
    def __init__(self, config, layer_idx):
        ...
        # Determine layer type from layer_types array
        layer_types = config.get("layer_types", [])
        layer_type_str = layer_types[layer_idx] if layer_idx < len(layer_types) else "conv"
        self.is_attention = layer_type_str == "full_attention"
...
class LFM2_5(nn.Module):
    """
    LFM2.5 (LiquidAI Foundation Model 2.5).

    Hybrid architecture with gated short-range convolution and attention blocks.
    Layer types determined by layer_types config parameter.
    """
    ...
    def from_pretrained(model_id):
        return from_pretrained(LFM2_5, model_id)
