"""
LLaMA 3 - Pure PyTorch implementation.

Usage:
    from models.llama.llama3 import Llama3

    model, tokenizer = Llama3.from_pretrained("meta-llama/Llama-3.2-1B")

    ids = torch.tensor([tokenizer.encode("Hello").ids])
    logits = model(ids)

Key differences from LLaMA 2:
- Higher rope_theta (500000 vs 10000) for longer context
- Larger vocabulary (128K tokens with tiktoken)
- Different tokenizer format

Supports: LLaMA 3, LLaMA 3.1, LLaMA 3.2
"""
...
class RotaryEmbedding(nn.Module):
    def __init__(self, dim, base=500000.0):  # LLaMA 3 uses higher base
        super().__init__()
...
class Llama3(nn.Module):
    def __init__(self, config):
        ...
        rope_theta = config.get("rope_theta", 500000.0)  # LLaMA 3 default
        eps = config.get("rms_norm_eps", 1e-5)  # LLaMA 3 uses 1e-5
        ...
        if config.get("tie_word_embeddings", False):  # LLaMA 3 usually doesn't tie
    ...
    def from_pretrained(model_id):
        return from_pretrained(Llama3, model_id)
