//! Collaboration storage adapter over process-local in-memory namespaces.
//!
//! TODO: persistent collab storage should attach to `TALU_DB_HOST`. For now
//! this is intentionally process-local and in-memory only.
...
pub const SessionStore = struct {
    allocator: Allocator,
    session_id: []u8,
    namespace: memory.NamespaceStore,
    ...
    pub fn init(allocator: Allocator, db_root: []const u8, session_id: []const u8) !SessionStore {
        ...
        return .{
            ...
            .namespace = try memory.NamespaceStore.init(allocator, namespace),
        };
    }
    ...
    pub fn namespaceId(self: *const SessionStore) []const u8 {
        return self.namespace.namespaceId();
    }
    ...
    pub fn putPresence(
        ...
        try self.namespace.putWithOptions(key, payload, .{
    ...
    pub fn getPresenceCopy(self: *SessionStore, allocator: Allocator, participant_id: []const u8) !?[]u8 {
        const key = try scopedKey(self.allocator, "presence", participant_id);
        defer self.allocator.free(key);
        return self.namespace.getCopy(allocator, key);
    }
    ...
    pub fn putParticipantState(self: *SessionStore, participant_id: []const u8, payload: []const u8) !void {
        ...
        defer self.allocator.free(key);
        try self.namespace.putWithOptions(key, payload, .{ .durability = .batched });
    }
    ...
    pub fn putCheckpoint(self: *SessionStore, name: []const u8, payload: []const u8) !void {
        ...
        defer self.allocator.free(key);
        try self.namespace.putWithOptions(key, payload, .{ .durability = .strong });
    }
    ...
    pub fn appendOperation(self: *SessionStore, op: types.OperationEnvelope) !void {
        ...
        defer self.allocator.free(key);
        try self.namespace.putWithOptions(key, op.payload, .{ .durability = .strong });
    }

    pub fn watchDrain(self: *SessionStore, allocator: Allocator, after_seq: u64, max_count: usize) !memory.WatchDrainResult {
        return self.namespace.watchDrain(allocator, after_seq, max_count);
    }

    pub fn flushBatched(self: *SessionStore) !void {
        try self.namespace.flushBatched();
    }

    pub fn flush(self: *SessionStore) !void {
        try self.namespace.flush();
    }

    pub fn stats(self: *SessionStore) memory.NamespaceStats {
        return self.namespace.stats();
    }

    pub fn setNowMsForTesting(self: *SessionStore, now_ms: ?i64) void {
        self.namespace.setNowMsForTesting(now_ms);
    }
};
...
test "SessionStore.putPresence uses ephemeral lane and default ttl" {
    var store = try SessionStore.init(std.testing.allocator, "ignored", "doc:1");
    defer store.deinit();
    ...
    try std.testing.expectEqual(memory.WatchEventType.put, events.events[0].event_type);
    try std.testing.expectEqual(@as(?memory.DurabilityClass, .ephemeral), events.events[0].durability);
...
test "SessionStore.putParticipantState queues batched writes and flushes" {
    var store = try SessionStore.init(std.testing.allocator, "ignored", "doc:2");
    defer store.deinit();
    ...
    var reopened = try SessionStore.init(std.testing.allocator, "ignored", "doc:2");
...
test "SessionStore.appendOperation persists strong operation envelope and emits watch event" {
    var store = try SessionStore.init(std.testing.allocator, "ignored", "doc:3");
    defer store.deinit();
    ...
    try std.testing.expectEqual(@as(usize, 1), events.events.len);
    try std.testing.expectEqual(memory.WatchEventType.put, events.events[0].event_type);
    try std.testing.expectEqual(@as(?memory.DurabilityClass, .strong), events.events[0].durability);
    try std.testing.expectEqualStrings("ops/agent:alpha:1:op-0001", events.events[0].key);

    var reopened = try SessionStore.init(std.testing.allocator, "ignored", "doc:3");
    defer reopened.deinit();
    const value = (try reopened.namespace.getCopy(std.testing.allocator, "ops/agent:alpha:1:op-0001")).?;
    defer std.testing.allocator.free(value);
