...
test "detectFromModelType resolves qwen3_vl alias from qwen3 graph metadata" {
    const allocator = std.testing.allocator;

    init(allocator);
    defer deinit();

    try loadFromFile("tools/archs/_graphs/qwen3.json");

    const detected = detectFromModelType("qwen3_vl");
    try std.testing.expect(detected != null);
    try std.testing.expectEqualStrings("qwen3", detected.?.name);

    const arch = detected.?;
    try std.testing.expect(arch.weight_prefixes.len > 0);

    var has_vl_prefix = false;
    for (arch.weight_prefixes) |prefix| {
        if (std.mem.eql(u8, prefix, "model.language_model.layers.{d}.")) {
            has_vl_prefix = true;
            break;
        }
    }
    try std.testing.expect(has_vl_prefix);

    var has_vl_embed_candidate = false;
    var has_vl_norm_candidate = false;
    for (arch.global_weights) |spec| {
        if (std.mem.eql(u8, spec.id, "token_embeddings")) {
            for (spec.candidates) |candidate| {
                if (std.mem.eql(u8, candidate, "model.language_model.embed_tokens.weight")) {
                    has_vl_embed_candidate = true;
                    break;
                }
            }
        }
        if (std.mem.eql(u8, spec.id, "ln_final")) {
            for (spec.candidates) |candidate| {
                if (std.mem.eql(u8, candidate, "model.language_model.norm.weight")) {
                    has_vl_norm_candidate = true;
                    break;
                }
            }
        }
    }
    try std.testing.expect(has_vl_embed_candidate);
    try std.testing.expect(has_vl_norm_candidate);
}

test "detectFromModelType resolves idefics3 alias from llama3 graph metadata" {
    const allocator = std.testing.allocator;

    init(allocator);
    defer deinit();

    try loadFromFile("tools/archs/_graphs/llama3.json");

    const detected = detectFromModelType("idefics3");
    try std.testing.expect(detected != null);
    try std.testing.expectEqualStrings("llama3", detected.?.name);

    const arch = detected.?;
    try std.testing.expect(arch.weight_prefixes.len > 0);

    var has_text_model_prefix = false;
    for (arch.weight_prefixes) |prefix| {
        if (std.mem.eql(u8, prefix, "model.text_model.layers.{d}.")) {
            has_text_model_prefix = true;
            break;
        }
    }
    try std.testing.expect(has_text_model_prefix);

    var has_text_embed_candidate = false;
    var has_text_norm_candidate = false;
    for (arch.global_weights) |spec| {
        if (std.mem.eql(u8, spec.id, "token_embeddings")) {
            for (spec.candidates) |candidate| {
                if (std.mem.eql(u8, candidate, "model.text_model.embed_tokens.weight")) {
                    has_text_embed_candidate = true;
                    break;
                }
            }
        }
        if (std.mem.eql(u8, spec.id, "ln_final")) {
            for (spec.candidates) |candidate| {
                if (std.mem.eql(u8, candidate, "model.text_model.norm.weight")) {
                    has_text_norm_candidate = true;
                    break;
                }
            }
        }
    }
    try std.testing.expect(has_text_embed_candidate);
    try std.testing.expect(has_text_norm_candidate);
}
