...
test "inference bridge stage_frame_header stageFrameHeaderFromMetadata copies tensor frame identity and payload facts" {
    var metadata = try testPrefillMetadata();
...
test "inference bridge stage_frame_header stageFrameHeaderFromMetadata derives host flags and rejects zero host ids" {
    var metadata = try testPrefillMetadata();
...
test "inference bridge stage_frame_header stageFrameHeaderFromMetadata rejects invalid metadata and multi batch metadata" {
...
test "inference bridge stage_frame_header encodeStageFrameHeader decodeStageFrameHeader round trips fixed little endian header and exact byte offsets" {
    const header = try testHeader();
    var encoded: [stage_frame_header_encoded_len]u8 = undefined;
    try encodeStageFrameHeader(&encoded, header);
...
test "inference bridge stage_frame_header encodeStageFrameHeader rejects invalid flags host ids tensor facts and payload byte count" {
...
test "inference bridge stage_frame_header decodeStageFrameHeader rejects wrong magic version length reserved bytes flags host ids identities enums tensor facts and payload byte count" {
...
test "inference bridge stage_frame_header validateStageFrameHeaderForMetadata rejects invalid metadata multi batch graph stage plan boundary tensor batch and byte count mismatches" {
    var metadata = try testPrefillMetadata();
