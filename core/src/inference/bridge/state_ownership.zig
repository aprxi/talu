...
test "inference bridge state_ownership buildStageStateOwnershipPlan validateStageStateOwnershipPlan descriptorSetForStage stateOwnershipPlanIdEql" {
...
test "inference bridge state_ownership validateStageStateDescriptorSet rejects invalid descriptors" {
...
test "inference bridge state_ownership validateStageStatePartitionFact preserves stateful dependencies without descriptor inference" {
    const allocator = std.testing.allocator;
...
test "inference bridge state_ownership validateStageStateOwnershipPlan rejects tampered copied partition summaries" {
    const allocator = std.testing.allocator;
    const dependencies = [_]stage_plan.DependencyOverride{.{
        .source_stage_id = 0,
        .target_stage_id = 1,
...
test "inference bridge state_ownership validateStageStatePartitionFact validates descriptor facts and unsupported partitions" {
    const allocator = std.testing.allocator;
    const dependencies = [_]stage_plan.DependencyOverride{.{
        .source_stage_id = 0,
        .target_stage_id = 1,
...
test "inference bridge state_ownership validateStageStateLease validateStateEpoch rejects identity and epoch mismatches" {
    const allocator = std.testing.allocator;
...
test "inference bridge state_ownership validatePrefillTransition validateDecodeTransition validateLeaseTransition follows state machine" {
    const allocator = std.testing.allocator;
    var stage_plan_value = try buildTestStagePlan(allocator, &.{2}, &.{});
    defer stage_plan_value.deinit();
    const sets = testDescriptorSets();
...
test "inference bridge state_ownership validateTensorFrameBatchEntryForStateLease validateTensorFrameForStateLeases enforces epoch and role" {
    const allocator = std.testing.allocator;
    var stage_plan_value = try buildTestStagePlan(allocator, &.{2}, &.{});
    defer stage_plan_value.deinit();
    const sets = testDescriptorSets();
...
test "inference bridge state_ownership validateStateDescriptorLifecycleAction shouldZeroStateDescriptorForLifecycleAction delegates lifecycle policy" {
...
test "inference bridge state_ownership buildStateCleanupObligations validateStateCleanupObligation is deterministic and idempotent" {
    const allocator = std.testing.allocator;
    var stage_plan_value = try buildTestStagePlan(allocator, &.{2}, &.{});
    defer stage_plan_value.deinit();
    const sets = testDescriptorSets();
...
test "inference bridge state_ownership validateStageStateBindingReports catches supplied alias only when reports exist" {
    const allocator = std.testing.allocator;
    var stage_plan_value = try buildTestStagePlan(allocator, &.{2}, &.{});
    defer stage_plan_value.deinit();
    const sets = testDescriptorSets();
