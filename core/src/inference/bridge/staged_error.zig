...
//! from the staged inference bridge contracts, builds deterministic cleanup
...
test "inference bridge staged_error classifyStagePlanError classifyTensorFrameError classifyStateOwnershipError classifyPlacementError classifyRunnerError classifyTransportError classifyCleanupCallbackError maps typed source errors" {
...
test "inference bridge staged_error buildStagedFailure validateStagedFailure rejects missing context and supplied contract mismatches" {
...
test "inference bridge staged_error validateStagedFailure rejects state ownership lifecycle mismatch" {
...
test "inference bridge staged_error buildStagedCleanupPlan validateStagedCleanupPlan stagedCleanupPlanIdEql stagedCleanupRequired orders idempotent single request cleanup" {
    var ownership_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer ownership_arena.deinit();
...
test "inference bridge staged_error buildStagedCleanupReport validateStagedCleanupReport stagedCleanupReportIdEql rejects lifecycle mismatch" {
    var ownership_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer ownership_arena.deinit();
    const descriptors = [_]runtime_contract.StateDescriptor{
        .{ .id = 1, .size_bytes = 16, .align_bytes = 8, .zero_init = true, .lifecycle = .request_scoped, .runtime_kind = 1 },
...
test "inference bridge staged_error buildStagedErrorReport validateStagedErrorReport attachCleanupFailures preserves primary failure" {
