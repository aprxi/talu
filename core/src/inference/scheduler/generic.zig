//! Generic Continuous Batching Scheduler
//!
...
const scheduler_contracts = @import("contracts.zig");
...
pub const RequestState = scheduler_contracts.RequestState;
pub const Request = scheduler_contracts.Request;
pub const FinishReason = scheduler_contracts.FinishReason;
pub const TokenEvent = scheduler_contracts.TokenEvent;
pub const TokenizerView = scheduler_contracts.TokenizerView;
pub const SchedulerConfig = scheduler_contracts.SchedulerConfig;
pub const SchedulerSingleDecodeRoute = scheduler_contracts.SchedulerSingleDecodeRoute;
pub const SchedulerSingleDecodeRoutePlan = scheduler_contracts.SchedulerSingleDecodeRoutePlan;
pub const SchedulerBatchedTopKRoutePlan = scheduler_contracts.SchedulerBatchedTopKRoutePlan;
...
pub fn GenericScheduler(comptime BackendType: type) type {
    return struct {
        ...
        prefill_requests: []PrefillBatchRequest,
        ...
        pub fn init(
            ...
            const prefill_request_buffer = try allocator.alloc(PrefillBatchRequest, effective_batch_size);
        ...
        pub const SubmitOptions = struct {
            ...
            /// Backend-specific vision prefill payload type.
        ...
        fn stepBatchedTopKCandidates(
            ...
            for (0..decode_batch_size) |row_idx| {
                ...
                var next_token = self.sampleTopKCandidateTokenForRequest(
                    request_entry,
                    self.decode_candidate_logits[row_start..row_end],
                    self.decode_candidate_ids[row_start..row_end],
                    candidate_count,
                ) catch 0;
                applyRequestThinkingBeforeAccept(request_entry, &next_token);
                ...
                const finish_reason = try self.acceptDecodeRequestToken(request_entry, next_token);
                const is_final_token = finish_reason != .in_progress;
                try self.emitStepRequestTokenEvent(
                    request_entry,
                    next_token,
                    finish_reason,
                    req.slot_index,
                    request_entry.in_thinking,
                );
        ...
        pub fn step(self: *Self) ![]const TokenEvent {
            ...
            // Fast path: when there is exactly one generating request, consult
            // backend-owned single-decode planning before taking top-k candidate
            // route. This keeps queued step() route selection aligned with
            // generateSync() and avoids bypassing backend crossover policy.
                ...
                if (use_topk) {
                    ...
                    if (topk_request) |request_entry| {
                        ...
                        const planned_route = self.backendPlanSingleDecodeRoute(&.{
                            ...
                            .capture_final_logits = request_entry.capture_final_logits,
                            .has_grammar_sampler = request_entry.grammar_sampler != null,
                            .prompt_token_count = request_entry.prompt_tokens.len,
                            .backend_streaming_semantic_eligible = false,
                            .top_k_candidate_semantic_eligible = topk_candidate_semantic_eligible,
                            .backend_streaming_backend_supported = false,
        ...
        fn stepTopKCandidate(
            ...
            try self.emitStepRequestTokenEvent(request_entry, next_token, finish_reason, slot_index, false);
        ...
        pub fn runDecodeLoop(
            ...
            while (re.generated_tokens.items.len < re.max_tokens) {
                ...
                const finish_reason = try self.acceptDecodeRequestToken(re, next_token);
                transitionRequestAnswerModeAfterAccept(re);
        ...
        pub fn generateSync(
            ...
            const top_k_candidate_semantic_eligible = prompt_tokens.len > 0 and
                self.active_requests.items.len == 0 and
                self.pending_queue.items.len == 0 and
                // Candidate route supports callbacks and remains available when
                // backend-owned tail streaming is unavailable for the config.
                submit_config.vision_input == null and
                submit_config.grammar_sampler == null and
                !submit_config.return_final_logits and
            ...
            const planned_route = self.backendPlanSingleDecodeRoute(&.{
                ...
                .capture_final_logits = submit_config.return_final_logits,
                .has_grammar_sampler = submit_config.grammar_sampler != null,
                .prompt_token_count = prompt_tokens.len,
                .backend_streaming_semantic_eligible = backend_streaming_semantic_eligible,
                .top_k_candidate_semantic_eligible = top_k_candidate_semantic_eligible,
                .backend_streaming_backend_supported = backend_streaming_backend_supported,
                .top_k_candidate_backend_supported = top_k_candidate_backend_supported,
            });
            switch (planned_route) {
                .backend_streaming => if (backend_streaming_semantic_eligible and backend_streaming_backend_supported) {
        ...
        fn generateSyncBackendStreamingRoute(
            self: *Self,
            prompt_tokens: []const u32,
            max_tokens: usize,
            ...
            if (try self.immediateGenerateSyncResult(max_tokens, submit_config.stop_flag)) |result| return result;

            var slot_binding = try self.bindSyncSlotState();
            defer slot_binding.deinit();
            const slot_index = slot_binding.slot_index;
            ...
            if (generationShouldStop(submit_config.stop_flag)) {
                return try self.emptyGenerateSyncResult(.cancelled, prefill_ns, 0);
            }

            const eos_token_ids = submit_config.eos_token_ids orelse self.config.default_eos_token_ids;
            const first_token = try self.sampleSyncFirstToken(max_tokens, submit_config, sampling_config, false);
            var generated = first_token.generated;
            errdefer generated.deinit(self.allocator);
            const current_token = first_token.token;

            if (first_token.finish_reason != .in_progress) {
                return try self.generatedSyncResult(&generated, first_token.finish_reason, prefill_ns, 0);
            }
            ...
            if (generationShouldStop(submit_config.stop_flag)) {
                return try self.generatedSyncResult(&generated, .cancelled, prefill_ns, 0);
            }
            ...
            if (sampling_config.strategy == .greedy and submit_config.stop_sequences.len > 0) {
            ...
            return try self.generatedSyncResult(&generated, finish_reason, prefill_ns, decode_ns);
        }
        ...
        fn generateSyncTopKCandidateRoute(
            ...
            const eos_token_ids = submit_config.eos_token_ids orelse self.config.default_eos_token_ids;
            var token_state = SyncTopKTokenState.init(submit_config);
            const first_token = try self.sampleSyncFirstToken(
                max_tokens,
                submit_config,
                sampling_config,
                token_state.in_thinking,
            );
            var generated = first_token.generated;
            errdefer generated.deinit(self.allocator);
            var current_token = first_token.token;
            var finish_reason = first_token.finish_reason;

            if (finish_reason != .in_progress) {
                return try self.generatedSyncResult(&generated, finish_reason, prefill_ns, 0);
            }
            ...
            var single_decode_request = [_]DecodeRequest{.{
                .slot_index = slot_index,
                .token = 0,
            }};
            ...
            while (generated.items.len < max_tokens) {
                if (generationShouldStop(submit_config.stop_flag)) {
                    return try self.generatedSyncResult(&generated, .cancelled, prefill_ns, decode_timer.read());
                }
                ...
                if (token_state.forcedThinkingToken()) |forced_token| {
                    ...
                    token_state.noteSampledThinkingToken(current_token);
                }
                finish_reason = try self.acceptSyncTopKCandidateToken(
                    &generated,
                    current_token,
                    &token_state,
                    max_tokens,
            ...
            return try self.generatedSyncResult(&generated, finish_reason, prefill_ns, decode_timer.read());
        }
        ...
        const SyncFirstToken = struct {
            generated: std.ArrayList(u32),
            token: u32,
            finish_reason: FinishReason,
        };

        const SyncStreamCallbackContext = struct {
        ...
        /// Local token lifecycle mirror for the synchronous top-k route, which
        /// does not allocate a queued Request.
        const SyncTopKTokenState = struct {
            ...
            generating_answer: bool,

            fn init(options: *const SubmitOptions) SyncTopKTokenState {
                const starts_in_thinking = options.max_thinking_tokens > 0 and options.thinking_end_tokens.len > 0;
                return .{
                    .max_thinking_tokens = options.max_thinking_tokens,
                    .thinking_end_tokens = options.thinking_end_tokens,
                    .in_thinking = starts_in_thinking,
                    .max_completion_tokens = options.max_completion_tokens,
                    .generating_answer = !starts_in_thinking,
                };
            }

            fn forcedThinkingToken(self: *SyncTopKTokenState) ?u32 {
                if (!self.in_thinking) return null;

                if (self.thinking_inject_pos > 0) {
                    const token = self.thinking_end_tokens[self.thinking_inject_pos];
                    self.thinking_inject_pos += 1;
                    if (self.thinking_inject_pos >= self.thinking_end_tokens.len) {
                        self.in_thinking = false;
                    }
                    return token;
                }

                if (self.thinking_token_count >= self.max_thinking_tokens) {
                    const token = self.thinking_end_tokens[0];
                    self.thinking_inject_pos = 1;
                    if (self.thinking_end_tokens.len == 1) {
                        self.in_thinking = false;
                    }
                    return token;
                }
            ...
            fn noteSampledThinkingToken(self: *SyncTopKTokenState, token: u32) void {
                if (!self.in_thinking) return;
                self.thinking_token_count += 1;
                if (self.thinking_end_tokens.len > 0 and token == self.thinking_end_tokens[0]) {
                    self.in_thinking = false;
                }
            }

            fn acceptToken(self: *SyncTopKTokenState, has_visible_output: bool) bool {
                if (self.generating_answer) {
                    if (has_visible_output) self.completion_token_count += 1;
                    if (self.max_completion_tokens > 0 and
                        self.completion_token_count >= self.max_completion_tokens)
                    {
                        return true;
                    }
                }
                if (!self.in_thinking and !self.generating_answer) {
        ...
        const SyncSlotStateBinding = struct {
            scheduler: *Self,
            slot_index: usize,
        ...
        fn immediateGenerateSyncResult(
            self: *Self,
            max_tokens: usize,
            stop_flag: ?*const std.atomic.Value(bool),
        ) !?GenerateSyncResult {
            if (max_tokens == 0) {
                return try self.emptyGenerateSyncResult(.length, 0, 0);
            }
            if (generationShouldStop(stop_flag)) {
                return try self.emptyGenerateSyncResult(.cancelled, 0, 0);
            }
            return null;
        }

        fn resolveSyncGeneratedTokenFinish(
            token: u32,
            ...
            for (eos_token_ids) |eos_id| {
                if (token == eos_id) return .eos_token;
            }

            if (stop_sequences.len > 0) {
                const stop_len = checkStopSequence(generated_tokens.items, stop_sequences);
                if (stop_len > 0) {
                    generated_tokens.shrinkRetainingCapacity(generated_tokens.items.len - stop_len);
        ...
        fn sampleSyncFirstToken(
            self: *Self,
            max_tokens: usize,
            submit_config: *const SubmitOptions,
            sampling_config: *const sampling.SamplingConfig,
            callback_in_thinking: bool,
        ) !SyncFirstToken {
            const eos_token_ids = submit_config.eos_token_ids orelse self.config.default_eos_token_ids;
            var sample_config = sampling_config.*;
            sample_config.context_tokens = &.{};
            const token = self.sampleToken(self.logits_buffer, sample_config, null) catch 0;

            var generated = try std.ArrayList(u32).initCapacity(self.allocator, max_tokens);
            errdefer generated.deinit(self.allocator);
            try generated.append(self.allocator, token);

            var finish_reason = resolveSyncGeneratedTokenFinish(
                token,
                &generated,
                eos_token_ids,
                submit_config.stop_sequences,
            );

            if (submit_config.callback) |cb| {
                const is_final = finish_reason != .in_progress or max_tokens == 1;
                if (finish_reason != .stop_sequence) {
                    cb(0, token, is_final, callback_in_thinking, submit_config.callback_data);
                }
            }

            if (finish_reason == .in_progress and max_tokens == 1) {
        ...
        fn acceptSyncTopKCandidateToken(
            self: *Self,
            generated: *std.ArrayList(u32),
            token: u32,
            token_state: *SyncTopKTokenState,
            max_tokens: usize,
            eos_token_ids: []const u32,
            submit_config: *const SubmitOptions,
        ) !FinishReason {
            ...
            if (token_state.acceptToken(self.tokenHasVisibleOutput(token))) {
                if (submit_config.callback) |cb| {
                    cb(0, token, true, token_state.in_thinking, submit_config.callback_data);
                }
                return .length;
            }
            ...
            if (submit_config.callback) |cb| {
                if (finish_reason != .stop_sequence) {
                    cb(0, token, finish_reason != .in_progress, token_state.in_thinking, submit_config.callback_data);
                }
            }
        ...
        fn bindSyncSlotState(self: *Self) !SyncSlotStateBinding {
            ...
            const slot_blocks = if (self.slot_persistent_descs.len > 0)
                ...
                null;
            var request_blocks = try self.allocateRequestStateBlocks();
            errdefer {
                self.applyLifecycleActionToRequestStateBlocks(&request_blocks, .evict) catch {};
                request_blocks.deinit(self.allocator);
            }

            var merged: [runtime_contract.max_state_descriptors]runtime_contract.StateBlockHandle = undefined;
            var merge_count: usize = 0;
            if (slot_blocks) |blocks| {
                for (blocks.handles) |handle| {
                    merged[merge_count] = handle;
                    merge_count += 1;
                }
            }
            for (request_blocks.handles) |handle| {
                merged[merge_count] = handle;
                merge_count += 1;
            }
            ...
            return .{
                .scheduler = self,
                .slot_index = slot_index,
                .request_state_blocks = request_blocks,
            };
        }

        fn tokenHasVisibleOutput(self: *const Self, token_id: u32) bool {
            return if (self.config.tokenizer) |tok|
                (tok.tokenBytes(token_id) orelse &.{}).len > 0
            else
                true;
        }

        fn forceRequestThinkingToken(request_entry: *Request) ?u32 {
            if (!request_entry.in_thinking) return null;

            if (request_entry.thinking_inject_pos > 0) {
                const token = request_entry.thinking_end_tokens[request_entry.thinking_inject_pos];
            ...
            if (request_entry.thinking_token_count >= request_entry.max_thinking_tokens) {
                const token = request_entry.thinking_end_tokens[0];
                request_entry.thinking_inject_pos = 1;
        ...
        fn noteSampledRequestThinkingToken(request_entry: *Request, token: u32) void {
            if (request_entry.in_thinking) {
                request_entry.thinking_token_count += 1;
                if (request_entry.thinking_end_tokens.len > 0 and
                    token == request_entry.thinking_end_tokens[0])
                {
                    request_entry.in_thinking = false;
                    request_entry.generating_answer = true;
                }
        ...
        fn applyRequestThinkingBeforeAccept(request_entry: *Request, token: *u32) void {
            if (forceRequestThinkingToken(request_entry)) |forced_token| {
                token.* = forced_token;
                return;
            }
            noteSampledRequestThinkingToken(request_entry, token.*);
            if (!request_entry.in_thinking and !request_entry.generating_answer) {
                request_entry.generating_answer = true;
            }
        }

        fn transitionRequestAnswerModeAfterAccept(request_entry: *Request) void {
            if (!request_entry.in_thinking and !request_entry.generating_answer) {
                request_entry.generating_answer = true;
            }
        }

        fn sampleTopKCandidateTokenForRequest(
            self: *Self,
            request_entry: *Request,
            candidate_logits: []f32,
            candidate_ids: []u32,
            ...
            var sample_cfg = request_entry.sampling_config;
            sample_cfg.context_tokens = request_entry.generated_tokens.items;
            return if (sampling_policy.canUseDirectGreedyCandidate(sample_cfg, candidate_count))
                candidate_ids[0]
            else
                try self.sampleTopKCandidateToken(
                    candidate_logits[0..candidate_count],
        ...
        fn sampleDecodeLoopTopKRequestToken(
            self: *Self,
            request_entry: *Request,
            candidate_logits: []f32,
            candidate_ids: []u32,
            ...
            if (forceRequestThinkingToken(request_entry)) |forced_token| return forced_token;
            const token = try self.sampleTopKCandidateTokenForRequest(
                request_entry,
                candidate_logits,
                candidate_ids,
                candidate_count,
        ...
        fn resolveRequestStopOrEos(request_entry: *Request, token: u32) FinishReason {
        ...
        fn consumeRequestCompletionBudget(self: *Self, request_entry: *Request, token: u32) bool {
            if (!request_entry.generating_answer) return false;
            if (request_entry.max_completion_tokens_limit == 0) return false;
            if (!self.tokenHasVisibleOutput(token)) return false;

            request_entry.completion_token_count += 1;
            return request_entry.completion_token_count >= request_entry.max_completion_tokens_limit;
        }

        fn acceptDecodeRequestToken(self: *Self, request_entry: *Request, token: u32) !FinishReason {
            try request_entry.generated_tokens.append(self.allocator, token);
            request_entry.token_position += 1;

            var finish_reason = resolveRequestStopOrEos(request_entry, token);
            if (finish_reason == .in_progress and self.consumeRequestCompletionBudget(request_entry, token)) {
                finish_reason = .length;
            }
        ...
        fn acceptPrefillRequestToken(self: *Self, request_entry: *Request, token: u32) !FinishReason {
            try request_entry.generated_tokens.append(self.allocator, token);
        ...
        fn emitStepRequestTokenEvent(
            self: *Self,
            request_entry: *Request,
            token: u32,
            finish_reason: FinishReason,
            slot_index: usize,
            event_in_thinking: bool,
        ) !void {
            const is_final = finish_reason != .in_progress;
            if (request_entry.callback) |cb| {
                if (finish_reason != .stop_sequence) {
                    cb(request_entry.id, token, is_final, request_entry.in_thinking, request_entry.callback_data);
                }
            }
        ...
        fn handlePrefillToken(
            ...
            const finish_reason = try self.acceptPrefillRequestToken(request_entry, first_token_id);
            const is_final_token = finish_reason != .in_progress;
...
test "init SubmitOptions defaults" {
    const opts = MockScheduler.SubmitOptions{};
...
test "init SubmitOptions custom priority" {
    const opts_high = MockScheduler.SubmitOptions{
        .priority = 100,
    };
    const opts_low = MockScheduler.SubmitOptions{
        .priority = -50,
    };
    const opts_default = MockScheduler.SubmitOptions{};
...
const MockBackend = struct {
    ...
    fn prefillBatch(self: *MockBackend, requests: []const PrefillBatchRequest) !void {
...
const MockStreamingBackend = struct {
    ...
    decode_batch_top_k_calls: usize = 0,
    decode_streaming_calls: usize = 0,
    decode_backend_streaming_top_k_calls: usize = 0,
    decode_top_k_calls: usize = 0,
    allow_top_k_decode_route: bool = true,
    ...
    fn supportsSchedulerBackendTopKDecodeRoute(
        ...
        if (!self.allow_top_k_decode_route) return false;
    ...
    fn planSchedulerSingleDecodeRoute(
        self: *const MockStreamingBackend,
    ...
    fn shouldUseSchedulerBatchedTopKDecodeRoute(
        self: *const MockStreamingBackend,
    ...
    fn decodeSchedulerStreaming(
        ...
        switch (sampling_config.strategy) {
            .greedy => self.decode_streaming_calls += 1,
            .top_k => self.decode_backend_streaming_top_k_calls += 1,
    ...
    fn decodeBatchTopKCandidates(
        ...
        self.decode_batch_top_k_calls += 1;
        if (top_k == 0) return error.InvalidArgument;
        for (requests, 0..) |_, row_idx| {
            if (row_idx >= candidate_counts_out.len) return error.InvalidArgument;
            const row_start = std.math.mul(usize, row_idx, top_k) catch return error.InvalidArgument;
            if (row_start >= candidate_logits_out.len or row_start >= candidate_ids_out.len) return error.InvalidArgument;
            candidate_logits_out[row_start] = 10.0;
            candidate_ids_out[row_start] = @intCast(self.greedy_token);
            candidate_counts_out[row_idx] = 1;
        }
    }
};
...
test "generateSync uses backend top-k streaming route for top_k sampling without penalties" {
    ...
    try std.testing.expectEqual(@as(usize, 1), backend.decode_backend_streaming_top_k_calls);
...
test "runDecodeLoop uses candidate route for top_k tail" {
    ...
    try std.testing.expectEqual(@as(usize, 0), backend.decode_backend_streaming_top_k_calls);
    try std.testing.expectEqual(@as(usize, 3), backend.decode_top_k_calls);
...
test "generateSync queued route skips batched top-k for single decode row" {
    const alloc = std.testing.allocator;
    var backend = MockStreamingBackend.init(alloc, 256);
    backend.allow_top_k_decode_route = false;
    backend.allow_batched_top_k_decode_route = true;
    defer backend.deinit();
    ...
    var callback_data = CallbackData{};
    const callback = struct {
        fn cb(_: u64, _: u32, _: bool, _: bool, user_data: ?*anyopaque) void {
            const data: *CallbackData = @ptrCast(@alignCast(user_data.?));
            data.token_count += 1;
        }
    }.cb;
    ...
    try std.testing.expectEqual(@as(usize, 0), backend.decode_batch_top_k_calls);
    try std.testing.expectEqual(@as(usize, 0), backend.decode_top_k_calls);
    try std.testing.expect(backend.decode_batch_calls > 0);
    try std.testing.expectEqual(@as(usize, 3), callback_data.token_count);
}

test "generateSync queued route honors backend-planned single-row queued policy even when top-k decode is supported" {
    const alloc = std.testing.allocator;
    var backend = MockStreamingBackend.init(alloc, 256);
    backend.allow_top_k_decode_route = true;
    backend.force_single_decode_route_queued = true;
    defer backend.deinit();
...
test "generateSync queued route can use backend-planned single-row batched top-k" {
    ...
    backend.allow_top_k_decode_route = false;
    backend.allow_batched_top_k_decode_route = true;
    backend.allow_single_row_batched_top_k_decode_route = true;
    defer backend.deinit();
    ...
    try std.testing.expect(backend.decode_batch_top_k_calls > 0);
    try std.testing.expectEqual(@as(usize, 0), backend.decode_top_k_calls);
    try std.testing.expectEqual(@as(usize, 0), backend.decode_batch_calls);
    try std.testing.expectEqual(@as(usize, 3), callback_data.token_count);
}
...
test "Scheduler.step - handles max tokens" {
    ...
    const prompt = [_]u32{ 1, 2, 3 };
    const request_id = try scheduler.submit(&prompt, 1, .{
        .eos_token_ids = &[_]u32{999},
    ...
    try std.testing.expectEqual(RequestState.completed, request_entry.state);
    try std.testing.expectEqual(@as(usize, 1), request_entry.generated_tokens.items.len);
    try std.testing.expectEqual(@as(usize, 0), request_entry.completion_token_count);
}
...
test "Scheduler.step - enforces max completion tokens on prefill token" {
    ...
    try std.testing.expectEqual(RequestState.completed, request_entry.state);
    try std.testing.expectEqual(@as(usize, 1), request_entry.generated_tokens.items.len);
    try std.testing.expectEqual(@as(usize, 1), request_entry.completion_token_count);
