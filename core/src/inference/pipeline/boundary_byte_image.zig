...
test "inference pipeline boundary_byte_image validateBoundaryByteImage accepts host readable exact byte image" {
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage accepts valid host_segments for host readable exact byte image" {
    const metadata = try testMetadata(.cpu);
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage accepts producer sync cpu image without host bytes" {
    const metadata = try testMetadata(.cpu);
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage accepts opaque local image when allowed" {
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects invalid contract version before metadata validation" {
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects host readable length mismatch" {
    const metadata = try testMetadata(.cpu);
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects invalid host_segments" {
    const metadata = try testMetadata(.cpu);
    const first = [_]u8{0xaa} ** 16;
    const second = [_]u8{0xbb} ** 48;
    const segments = [_][]const u8{ &first, &second };
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage accepts pending host_segments when allowed" {
    const metadata = try testMetadata(.cpu);
    const pending = [_]u8{};
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects invalid payload byte count" {
    const metadata = try testMetadata(.cpu);
    const payload = [_]u8{0xaa} ** 64;
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects missing host bytes when required" {
    const metadata = try testMetadata(.cpu);
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage accepts device resident byte image without forcing download" {
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects invalid readiness location combinations" {
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects opaque local when remote readable bytes are required" {
    const metadata = try testMetadata(.{ .opaque_local = 9 });
    const image = testImage(&metadata, .local_only_opaque, null);
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects payload location hint mismatches including absent hints" {
    const metadata = try testMetadata(.{ .cuda = 2 });
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage rejects payload ownership and lifetime mismatches" {
    const metadata = try testMetadata(.cpu);
...
test "inference pipeline boundary_byte_image validateBoundaryByteImage returns RemoteByteImageUnavailable for valid non remote readable images" {
    const cpu_metadata = try testMetadata(.cpu);
...
test "inference pipeline boundary_byte_image boundaryByteImageIsRemoteReadable returns true only for host readable exact bytes" {
    const cpu_metadata = try testMetadata(.cpu);
    const payload = [_]u8{0xaa} ** 64;
