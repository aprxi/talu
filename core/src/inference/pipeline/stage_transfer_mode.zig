...
test "inference pipeline stage_transfer_mode chooseStageTransferMode selects borrow_in_process for valid same host borrowed bytes" {
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode selects copy_in_process when borrowing is not allowed" {
    const allocator = std.testing.allocator;
    var bundle = try buildPlacementBundle(allocator, .same_host, .same_host_direct, .{});
    defer bundle.deinit();
    var metadata = try metadataForPlacement(&bundle.placement, .decode, .f32, .{ 1, 1, 4, 0 }, &decode_entries);
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode selects copy_in_process for segmented host readable bytes" {
    const allocator = std.testing.allocator;
    var bundle = try buildPlacementBundle(allocator, .same_host, .same_host_direct, .{});
    defer bundle.deinit();
    var metadata = try metadataForPlacement(&bundle.placement, .decode, .f32, .{ 1, 1, 4, 0 }, &decode_entries);
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode selects copy_in_process for local and mock host readable bytes" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode selects device_download_then_copy for local device resident bytes without peer copy" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode selects device_peer_copy_in_process for local device resident bytes when available" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode keeps cross backend device handoff host staged" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode does not select peer copy for mock or remote device resident bytes" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode selects remote_stream for remote host readable bytes" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode selects device_download_then_remote_stream for remote device resident bytes" {
    const allocator = std.testing.allocator;
    var bundle = try buildPlacementBundle(allocator, .two_host_remote, .remote_declared, .{});
    defer bundle.deinit();
    var metadata = try metadataForPlacement(&bundle.placement, .decode, .f32, .{ 1, 1, 4, 0 }, &decode_entries);
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode rejects producer sync and opaque local payloads for every handoff mode" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode rejects mismatched metadata image placement graph full boundary and dtype facts" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode rejects missing dtype mismatched and envelope mismatched boundary profiles" {
    const allocator = std.testing.allocator;
...
test "inference pipeline stage_transfer_mode chooseStageTransferMode returns exact placement errors for invalid profile cardinality" {
    const allocator = std.testing.allocator;
    var bundle = try buildPlacementBundle(allocator, .two_host_local, .local_in_process, .{});
    defer bundle.deinit();
    var metadata = try metadataForPlacement(&bundle.placement, .decode, .f32, .{ 1, 1, 4, 0 }, &decode_entries);
