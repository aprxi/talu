...
// Production pipeline users must enter through executeLocalStageChain so ordered
// local topology execution has one pipeline path.
...
test "buildDecodeActivationMetadata buildPrefillActivationMetadata hostActivationByteImage segmentedHostActivationByteImage deviceActivationByteImage buildActivationTransportContract builds pipeline handoff facts" {
    const allocator = std.testing.allocator;
    var fixture = try TestFixture.init(allocator, &.{2}, .local_in_process, false);
    defer fixture.deinit();
...
test "localStageAdapter executeLocalStageChain executes pipeline boundary" {
    const allocator = std.testing.allocator;
    var fixture = try TestFixture.init(allocator, &.{2}, .local_in_process, false);
    defer fixture.deinit();
    var runner = try buildLocalStageRunnerPlanRef(allocator, .{
