...
test "inference pipeline stage_transport buildStageTransportActivationEnvelope validateStageTransportEnvelope accepts adjacent activation payload" {
...
test "inference pipeline stage_transport buildStageTransportActivationEnvelope accepts peer copy only for local device resident activations" {
...
test "inference pipeline stage_transport buildStageTransportActivationEnvelope rejects impossible activation transfer decisions" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_transport validateStageTransportEnvelope rejects contract header failure transfer mode and byte count mismatches" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_transport writeStageTransportEnvelopeLocal readStageTransportEnvelopeLocal moves fixed header plus raw payload bytes" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_transport writeStageTransportEnvelopeLocal rejects non copy transfer modes and mismatched metadata before payload write" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_transport buildStageTransportFailureEnvelope validateStageTransportEnvelope accepts propagated staged failure without payload bytes" {
    const failure = transportFailure();
...
test "inference pipeline stage_transport buildStageTransportCancellationEnvelope validateStageTransportEnvelope accepts request_cancelled without payload bytes" {
...
test "inference pipeline stage_transport validates repeated adjacent activation envelopes for a three stage chain" {
