...
//!
//! This module derives ordered pipeline/runtime facts from an explicit local
...
const pipeline = struct {
    ...
    const negotiateBoundaryContract = pipeline_contract.negotiateBoundaryContract;
};
...
pub const StageRuntimeCapability = struct {
    ...
    supported_boundary_dtypes: []const pipeline.BoundaryDType,
};

pub const Plan = struct {
    allocator: std.mem.Allocator,
    split_points: []usize = &.{},
    stage_backend_kinds: []pipeline.HostBackendKind = &.{},
...
pub fn buildPlan(request: BuildRequest) !Plan {
    ...
    plan.stage_backend_kinds = try request.allocator.alloc(pipeline.HostBackendKind, request.stages.len);
    ...
    for (0..boundary_count) |boundary_index| {
        ...
        const boundary = try pipeline.negotiateBoundaryContract(.{
...
test "local_stage_runtime buildPlan builds generic mixed runtime facts" {
    ...
    try std.testing.expectEqual(pipeline.HostBackendKind.cpu, plan.stage_specs[0].backend_kind);
    try std.testing.expectEqual(pipeline.HostBackendKind.cuda, plan.stage_specs[1].backend_kind);
    try std.testing.expectEqual(pipeline.HostBackendKind.cpu, plan.stage_specs[2].backend_kind);
...
fn testCapability(stage_id: usize, kind: pipeline.HostBackendKind) StageRuntimeCapability {
    return .{
        .stage_id = stage_id,
        .backend_kind = kind,
        .max_batch_size = 4,
