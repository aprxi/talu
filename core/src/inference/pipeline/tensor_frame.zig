...
test "inference pipeline dtypeByteSize returns transfer element sizes" {
...
test "inference pipeline TensorFramePlanRef.fromStagePlan copies fixed identity and boundary summaries" {
...
test "inference pipeline TensorFramePlanRef.fromStagePlan rejects invalid StagePlan" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
...
test "inference pipeline boundaryRefFromPlanRef rejects missing and out-of-range boundaries" {
...
test "inference pipeline activationDecodeFrame builds a StagePlan-derived frame" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline activationDecodeFrame rejects missing selected boundary tensor contract" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline activationDecodeFrame rejects boundary tensor contract mismatch" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline validateTensorFramePlanIdentity rejects graph and stage plan mismatch" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline activation frame validates shape hidden width stride and step kind" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline activation frame validates byte counts and payload buffer length" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline activation frame rejects invalid frame id and unsupported compute dtypes" {
...
test "inference pipeline activation frame validates explicit non-ragged unique batch entries" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline activation frame rejects zero request slot and sequence overflow" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline payload location hint is excluded from logical equality and hash" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline payload ownership and lifetime validation rejects invalid combinations" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
...
test "inference pipeline emitTensorFrame no-op best-effort and strict observer behavior" {
    var fixture = try PlanFixture.init(std.testing.allocator, &.{2});
    defer fixture.deinit();
    var plan_ref = try TensorFramePlanRef.fromStagePlan(std.testing.allocator, &fixture.plan);
    defer plan_ref.deinit();
