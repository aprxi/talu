...
test "inference pipeline stage_byte_harness writeStageFrameBytes readStageFrameBytes copies header plus raw payload bytes" {
...
test "inference pipeline stage_byte_harness writeStageFrameBytes writes segmented host readable bytes in order" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_byte_harness writeStageFrameBytes rejects mismatched metadata and image before header write" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_byte_harness writeStageFrameBytes reports writer errors short header and short payload writes" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_byte_harness readStageFrameBytes rejects reader errors short header and short payload" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_byte_harness readStageFrameBytes rejects graph stage boundary and byte count mismatch before receiver mutation" {
    var metadata = try testDecodeMetadata();
...
test "inference pipeline stage_byte_harness readStageFrameBytes rejects destination buffer too small before receiver mutation" {
    var metadata = try testDecodeMetadata();
