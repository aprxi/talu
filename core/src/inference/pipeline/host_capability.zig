...
test "inference pipeline host_capability buildHostCapability validateHostCapability hostCapabilityIdEql boundary frame producer consumer validation" {
...
test "inference pipeline host_capability buildHostResidencySnapshot validateHostResidencySnapshot hostResidencySnapshotIdEql exact stage residency" {
    const allocator = std.testing.allocator;
...
test "inference pipeline host_capability buildStageStatePlacementRef validateStageStatePlacementRef copies descriptor summaries" {
    const allocator = std.testing.allocator;
...
test "inference pipeline host_capability buildPlacementPlan validatePlacementPlan placementPlanIdEql bindingForStage stageIdsForHost hostSummaryForStage two host placement" {
    const allocator = std.testing.allocator;
...
test "inference pipeline host_capability buildPlacementPlan accepts single stage placement and rejects stateless resident state facts" {
    const allocator = std.testing.allocator;
...
test "inference pipeline host_capability validateBoundaryFrameProfileCardinality and placement validation reject missing residency duplicate profiles budget and remote reachability" {
    const allocator = std.testing.allocator;
    var plan = try buildTestStagePlan(allocator, &.{2}, &.{});
    defer plan.deinit();
    var cap0 = try buildTestCapability(allocator, testHostId(1), .local_in_process, .local_in_process);
...
test "inference pipeline host_capability buildPlacementPlan rejects duplicate inputs step sets binding errors and identity mismatches" {
    const allocator = std.testing.allocator;
    var plan = try buildTestStagePlan(allocator, &.{2}, &.{});
    defer plan.deinit();
    var cap0 = try buildTestCapability(allocator, testHostId(1), .local_in_process, .local_in_process);
...
test "inference pipeline host_capability buildPlacementPlan rejects unsupported boundary capability combinations" {
    const allocator = std.testing.allocator;
    var plan = try buildTestStagePlan(allocator, &.{2}, &.{});
    defer plan.deinit();
    var cap0 = try buildTestCapability(allocator, testHostId(1), .local_in_process, .local_in_process);
...
test "inference pipeline host_capability placementPlanIdEql changes with binding capability residency profile and state summary" {
    const allocator = std.testing.allocator;
    var plan = try buildTestStagePlan(allocator, &.{2}, &.{});
    defer plan.deinit();
    var cap0 = try buildTestCapability(allocator, testHostId(1), .local_in_process, .local_in_process);
...
test "inference pipeline host_capability buildPlacementPlan validates same host and state placement modes" {
    const allocator = std.testing.allocator;
    const dependencies = [_]stage_plan.DependencyOverride{.{
        .source_stage_id = 0,
        .target_stage_id = 1,
