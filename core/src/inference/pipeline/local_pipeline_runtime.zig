//! Pipeline-owned local pipeline runtime.
//!
//! Concrete backends provide erased stage handles. This runtime owns the
//! ordered stage list, pipeline contracts, slot/request bookkeeping, and
//! activation handoff requests. Backends execute only their configured layer
//! ranges and expose external activation input/output surfaces through the
//! stage vtable.
...
const sampling_policy = @import("../sampling/policy.zig");
...
pub const StageVTable = struct {
    ...
    decode_external_output: *const fn (*anyopaque, usize, usize) anyerror!transport.ExternalActivationSource,
    prefill_external_output: *const fn (*anyopaque, usize) anyerror!transport.ExternalActivationSource,
    decode_external_input: *const fn (*anyopaque, usize, usize) anyerror!transport.ExternalActivationTarget,
    prefill_external_input: *const fn (*anyopaque, usize) anyerror!transport.ExternalActivationTarget,
...
fn canPeerCopyStages(source: *const StageHandle, target: *const StageHandle) bool {
...
pub const LocalPipelineRuntime = struct {
    ...
    pub fn init(request: InitRequest) !LocalPipelineRuntime {
        ...
        var stages_owned_by_init = true;
        errdefer if (stages_owned_by_init) {
            for (request.stages) |*stage| stage.deinit(request.allocator);
            request.allocator.free(request.stages);
        };

        const boundary_peer_copy_available = try request.allocator.alloc(bool, request.stages.len - 1);
        defer request.allocator.free(boundary_peer_copy_available);
        for (boundary_peer_copy_available, 0..) |*available, boundary_index| {
            available.* = canPeerCopyStages(
                &request.stages[boundary_index],
                &request.stages[boundary_index + 1],
            );
        }
    ...
    pub fn decodeTopKCandidates(
        ...
        return sampling_policy.extractTopKFromHostLogitsRow(logits, top_k, candidate_logits_out, candidate_ids_out);
    }
    ...
    fn decodeBoundaryPayloads(
        ...
        for (out[0..boundary_count], 0..) |*payload, boundary_index| {
            ...
            const output = try source_stage.vtable.decode_external_output(source_stage.ptr, slot_indices[0], row_bytes);
            payload.* = .{
                ...
                .image = decodeExternalOutputImage(output),
            };
        }
    ...
    fn prefillBoundaryPayloads(
        ...
        for (out[0..boundary_count], 0..) |*payload, boundary_index| {
            ...
            const output = try source_stage.vtable.prefill_external_output(source_stage.ptr, transfer_bytes);
            payload.* = .{
                .frame = try self.boundaryFrame(boundary_index),
                .slot_index = slot_index,
                .sequence_start = sequence_start,
                ...
                .activation_byte_count = transfer_bytes,
                .location_hint = sourceLocationHint(source_stage),
                .image = prefillExternalOutputImage(output),
            };
        }
        return out[0..boundary_count];
    }

    fn decodeExternalOutputImage(source: transport.ExternalActivationSource) local_pipeline.LocalDecodeBoundaryImageSpec {
        return switch (source) {
            .host => |bytes| .{ .host_bytes = bytes },
            .cuda => .device,
        };
    }

    fn prefillExternalOutputImage(source: transport.ExternalActivationSource) local_pipeline.LocalPrefillBoundaryImageSpec {
        return switch (source) {
            .host => |bytes| .{ .host_bytes = bytes },
            .cuda => .device,
        };
...
const PipelineEndpoint = struct {
    ...
    fn externalOutput(self: *@This(), byte_count: usize) anyerror!transport.ExternalActivationSource {
        return switch (self.mode) {
            .decode => |request| self.stage().vtable.decode_external_output(
                self.stage().ptr,
                request.slot_index,
                byte_count,
            ),
            .prefill => self.stage().vtable.prefill_external_output(self.stage().ptr, byte_count),
        };
    }

    fn externalInput(self: *@This(), byte_count: usize) anyerror!transport.ExternalActivationTarget {
        return switch (self.mode) {
            .decode => |request| self.stage().vtable.decode_external_input(
                self.stage().ptr,
                request.slot_index,
                byte_count,
            ),
            .prefill => self.stage().vtable.prefill_external_input(self.stage().ptr, byte_count),
        };
    }
    ...
    fn downloadActivation(ptr: *anyopaque, host_buf: []u8, byte_count: usize) anyerror!void {
        const self: *PipelineEndpoint = @ptrCast(@alignCast(ptr));
        try transport.downloadExternalActivation(try self.externalOutput(byte_count), host_buf, byte_count);
    }

    fn uploadActivation(ptr: *anyopaque, host_buf: []const u8, byte_count: usize) anyerror!void {
        const self: *PipelineEndpoint = @ptrCast(@alignCast(ptr));
        try transport.uploadExternalActivation(try self.externalInput(byte_count), host_buf, byte_count);
    }

    fn uploadActivationSegments(ptr: *anyopaque, host_segments: []const []const u8, byte_count: usize) anyerror!void {
        const self: *PipelineEndpoint = @ptrCast(@alignCast(ptr));
        try transport.uploadExternalActivationSegments(try self.externalInput(byte_count), host_segments, byte_count);
    }
    ...
    fn peerCopyActivationTo(ptr: *anyopaque, target_ptr: *anyopaque, byte_count: usize) anyerror!void {
        ...
        try transport.peerCopyExternalActivation(
            try self.externalOutput(byte_count),
            try target.externalInput(byte_count),
            byte_count,
        );
    }
    ...
    fn peerCopyHandlesStageSync(ptr: *anyopaque) bool {
        const self: *PipelineEndpoint = @ptrCast(@alignCast(ptr));
        return transport.externalActivationPeerCopyHandlesStageSync(self.externalOutput(0) catch return false);
    }
