...
const fused = @import("backend/cpu/fused.zig");
const FusedCpuBackend = fused.FusedCpuBackend;
const DecodeRequest = fused.DecodeRequest;
const DecodeResult = fused.DecodeResult;
...
pub fn GenericScheduler(comptime BackendType: type) type {
    return struct {
        ...
        fn generateSyncFastPath(
            ...
            // Decode loop - uses decodeBatch for all cases (single or multi-request)
