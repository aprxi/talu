...
const vision_runtime_mod = @import("../cpu/vision_runtime.zig");
...
pub const MetalBackend = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, loaded: *model_loader.LoadedModel) !MetalBackend {
        ...
        const logits_buffer = try allocator.alloc(f32, @intCast(loaded.config.vocab_size));
    ...
    /// Allocate scheduler slot (Metal currently supports one active sequence).
    ...
    pub fn prefillSlot(
        ...
        return self.prefill(tokens, logits_out);
    }
    ...
    pub fn prefillSlotWithVision(
        ...
        if (slot_index != 0) return error.InvalidArgument;
        if (vision_input == null) return self.prefill(tokens, logits_out);
        ...
        const embedding_handle = try mlx_forward.gatherTokenEmbeddingsLazy(self.weights, tokens);
        ...
        var deepstack_ctx: ?mlx_forward.DeepstackAdditions = null;
        ...
        var runtime_rope_ctx: ?mlx_forward.RuntimeRoPEOverride = null;
        const head_dim: usize = @intCast(self.config.head_dim);
        const mrope_section = resolveMropeSection(&self.config, head_dim);
        if (mrope_section[0] + mrope_section[1] + mrope_section[2] > 0) {
            ...
            self.slot_rope_position_delta = tables.position_delta;
            runtime_rope_ctx = .{
            ...
            log.debug("inference", "Metal text MRoPE prepared", .{
                ...
                .position_delta = self.slot_rope_position_delta,
            }, @src());
        }
        ...
        const logits_handle = try mlx_forward.transformerForwardLazyWithEmbeddingOverride(
            self.allocator,
            self.weights,
            tokens,
            self.config,
    ...
    pub fn decodeBatch(self: *MetalBackend, requests: anytype, results: anytype) !void {
        if (requests.len == 0) return;
        if (results.len < requests.len) return error.InvalidArgument;

        for (requests, 0..) |request, index| {
            if (request.slot_index != 0) return error.InvalidArgument;
            try self.decode(request.token, self.current_position, self.logits_buffer);
            results[index] = .{
    ...
    pub fn decode(self: *MetalBackend, token: u32, position: usize, logits_out: []f32) !void {
        const effective_position = try applyPositionDelta(position, self.slot_rope_position_delta);
    ...
    pub fn decodeStreaming(
        ...
        if (batch_decode_enabled and use_fused_path) {
            const effective_start_position = try applyPositionDelta(start_position, self.slot_rope_position_delta);
    ...
    fn decodeStreamingFused(
        ...
        // Prime the pipeline
        const effective_start_position = try applyPositionDelta(position_index, self.slot_rope_position_delta);
        if (self.fused_model) |fused| {
        ...
        while (generated_count < max_tokens) : (generated_count += 1) {
            ...
            if (generated_count + 1 < max_tokens) {
                ...
                const effective_position = try applyPositionDelta(position_index, self.slot_rope_position_delta);
                if (self.fused_model) |fused| {
...
test "applyPositionDelta rejects negative resulting positions" {
    try std.testing.expectError(error.InvalidShape, MetalBackend.applyPositionDelta(2, -3));
    try std.testing.expectEqual(@as(usize, 7), try MetalBackend.applyPositionDelta(4, 3));
}
