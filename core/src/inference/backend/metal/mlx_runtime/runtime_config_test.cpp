...
void test_parse_model_config_accepts_shared_expert_intermediate_size() {
    const auto dir = write_config_dir(
        "talu-mlx-runtime-config-shared-expert",
        R"JSON({
  "architectures": ["Qwen3_5MoeForConditionalGeneration"],
  "model_type": "qwen3_5_moe",
  "text_config": {
...
void test_parse_model_config_flags_apply_identity_values() {
    const auto dir = write_config_dir(
        "talu-mlx-runtime-config-flags-identity",
        R"JSON({
  "text_config": {
    "hidden_size": 2048,
    "num_hidden_layers": 24,
...
void test_parse_model_config_reads_moe_routing_fields() {
    const auto dir = write_config_dir(
        "talu-mlx-runtime-config-routing",
        R"JSON({
  "architectures": ["Qwen3_5MoeForConditionalGeneration"],
...
int main() {
    try {
        ...
        std::cout << "runtime_config_test: ok\n";
        return 0;
    } catch (const std::exception& err) {
        std::cerr << "runtime_config_test: " << err.what() << "\n";
        return 1;
    }
}
