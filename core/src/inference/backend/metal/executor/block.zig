...
pub const TransformerBlock = struct {
        ...
        const output = switch (template) {
            .mla => |mla_template| blk: {
                ...
                var attention = mla_kernel.MLAttention{
                    ...
                    .q_a_norm = mla_template.q_a_norm,
                    .kv_a_norm = mla_template.kv_a_norm,
                    .o_proj = null,
                    .o_proj_bf16 = null,
                };
                switch (mla_template.storage_kind) {
                    .quantized => attention.o_proj = quantizedWeightFromHandle(weight_handles[0]).*,
                    .dense => attention.o_proj_bf16 = arrayWeightFromHandle(weight_handles[0]).*,
    ...
    fn layerProgramMambaAdapter(
        ...
        var mamba = mamba_kernel.MambaKernel{
            ...
            .in_proj = null,
            .in_proj_bf16 = null,
            .conv_weight = template.conv_weight,
            .conv_bias = template.conv_bias,
            .a_log = template.a_log,
            .d_skip = template.d_skip,
        ...
        if (template.storage_kind == .quantized) {
            ...
            mamba.out_proj = quantizedWeightFromHandle(weight_handles[1]).*;
        } else if (template.storage_kind == .dense) {
            mamba.in_proj_bf16 = arrayWeightFromHandle(weight_handles[0]).*;
            mamba.out_proj_bf16 = arrayWeightFromHandle(weight_handles[1]).*;
        } else {
