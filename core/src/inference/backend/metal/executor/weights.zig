//! Metal weight-loading lifecycle.
//!
//! Owns GPU weight materialization (`WeightHandles`) for the Metal backend.
...
fn tensorToArray(tensor: *const Tensor) MLXError!ArrayHandle {
    ...
    switch (tensor.dtype) {
        ...
        .bf16 => {
            ...
            return mlx_graph.createArrayBF16DenseWeightUnaligned(data_ptr, element_count, shape);
        },
        .f16 => {
            const element_count = tensor.data_size / 2;
            const data_ptr: [*]align(1) const u16 = @ptrCast(tensor.data_ptr);
...
fn loadNormWeight(weight: *const Tensor) MLXError!ArrayHandle {
    switch (weight.dtype) {
        ...
        .bf16 => {
            ...
            const shape = [_]i64{@intCast(element_count)};
            const data_ptr: [*]align(1) const u16 = @ptrCast(weight.data_ptr);
            return mlx_graph.createArrayBF16NormUnaligned(data_ptr, element_count, &shape);
        },
        else => return error.UnsupportedDType,
    }
}
...
fn resolveLayerInstructionWeightPtrForSlot(
    ...
    switch (opcode) {
        ...
        .moe => {
            ...
            return @ptrCast(@constCast(&moe_ptr.router_w));
        },
        .mamba_mixer => {
            return switch (slot_idx) {
                0 => if (layer.mamba_in_proj) |*weight|
                    ...
                    error.MissingWeight,
                1 => if (layer.mamba_out_proj) |*weight|
                    @ptrCast(weight)
