...
//!
//! Builds lazy computation graphs where operations don't execute until
//! mx.eval() is called. Copies only logits to CPU for sampling.
...
pub fn transformerForwardLazyWithEmbeddingOverride(
    ...
    for (0..layer_count) |layer_idx| {
        ...
        const layer_weights = weight_handles.layers[layer_idx];
        ...
        // 2-10. >>> LAZY: Layer mixer (attention or shortconv)
        const mixer_out = switch (layer_weights.kind) {
            .attention_mlp => if (layer_weights.q_proj != null and layer_weights.k_proj != null and layer_weights.v_proj != null) blk: {
                const cache_handle = if (cache) |c| c.handle else null;
                const q_proj = layer_weights.q_proj.?;
                if (layer_weights.o_proj) |o_proj| {
                ...
                break :blk mlx_graph.mlx_lazy_fused_attention_qkv_quantized_o_dense(
                    normed,
                    q_proj.weights,
                    q_proj.scales,
                    q_proj.biases,
            ...
            .shortconv => blk: {
                const sc_cache_handle = if (shortconv_cache) |sc| sc.handle else null;
                if (layer_weights.shortconv_in_proj) |in_proj_q| {
                    const out_proj_q = layer_weights.shortconv_out_proj orelse return error.MissingField;
                    break :blk mlx_graph.mlx_lazy_shortconv_mixer_quantized(
                        normed,
                        in_proj_q.weights,
                        in_proj_q.scales,
                        in_proj_q.biases,
                        layer_weights.shortconv_conv_weight.?,
                        if (layer_weights.shortconv_conv_bias) |b| b else null,
                        out_proj_q.weights,
                        out_proj_q.scales,
                        out_proj_q.biases,
                        in_proj_q.group_size,
                        in_proj_q.bits,
                        sc_cache_handle,
                        layer_idx,
                        layer_weights.shortconv_d_conv,
                        layer_weights.shortconv_conv_dim,
                    );
                }
                break :blk mlx_graph.mlx_lazy_shortconv_mixer_bf16(
                    normed,
                    layer_weights.shortconv_in_proj_bf16.?,
                    layer_weights.shortconv_conv_weight.?,
                    if (layer_weights.shortconv_conv_bias) |b| b else null,
                    layer_weights.shortconv_out_proj_bf16.?,
                    sc_cache_handle,
                    layer_idx,
                    layer_weights.shortconv_d_conv,
                    layer_weights.shortconv_conv_dim,
        ...
        const attn_for_residual = if (weight_handles.use_post_attn_norm)
            mlx_graph.mlx_lazy_rms_norm(mixer_out, layer_weights.ln2_weight, norm_eps)
        else
            mixer_out;
        // Scale layer output by residual_multiplier before adding to residual
        ...
        // Use pre_ffn_norm if present, otherwise use ln2_weight
        const normed_2 = if (weight_handles.use_post_attn_norm and layer_weights.pre_ffn_norm != null)
        ...
        } else if (layer_weights.w1 != null and layer_weights.w2 != null and layer_weights.w3 != null) blk: {
            // Quantized FFN path
        ...
        } else if (layer_weights.w1_bf16 != null and layer_weights.w2_bf16 != null and layer_weights.w3_bf16 != null) blk: {
            // Dense FFN path
        ...
        const ffn_for_residual = if (layer_weights.post_ffn_norm) |post_ffn|
            mlx_graph.mlx_lazy_rms_norm(ffn_out, post_ffn, norm_eps)
        else
            ffn_out;
        // Scale layer output by residual_multiplier before adding to residual
    ...
    const logits = if (weight_handles.lm_head_quantized) |quantized_lm_head| blk: {
        ...
        // Non-quantized matmul - transpose if lm_head is stored as (vocab_size, d_model)
    ...
    // Divide logits by logits_scaling (e.g., Granite)
    const scaled_logits = if (weight_handles.logits_scaling != 1.0)
...
/// Transformer forward pass with a GPU token handle (for pipelined generation).
/// Builds the graph lazily without waiting for the token value.
/// Mirrors transformerForwardLazy but uses mlx_lazy_embedding_from_array.
...
const block_kernels = @import("../cpu/block_kernels.zig");
...
test "transformerForwardLazy produces logits from input tokens" {
    if (comptime builtin.os.tag != .macos) return;
    if (!metal_device.isAvailable()) return;
    ...
    const logits_handle = try transformerForwardLazy(
        testing.allocator,
        weight_handles,
        &input_ids,
        loaded.config,
...
test "transformerForwardFromGPUToken produces logits from GPU token" {
    if (comptime builtin.os.tag != .macos) return;
    if (!metal_device.isAvailable()) return;
    ...
    const prefill_logits = try transformerForwardLazy(
        testing.allocator,
        weight_handles,
        &prefill_ids,
        loaded.config,
    ...
    const logits_handle = try transformerForwardFromGPUToken(
        testing.allocator,
        weight_handles,
        token_handle,
        loaded.config,
