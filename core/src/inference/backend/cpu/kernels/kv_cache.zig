...
pub const BatchedKVCache = struct {
    ...
    pub fn getV(self: *const BatchedKVCache, slot_index: usize, kv_head: usize, position: usize) []f32 {
        if (self.quant_mode != .f32) @panic("getV called in int8 mode - use dequantization path");
        const offset = self.kvOffset(slot_index, kv_head, position);
...
test "BatchedKVCache: initWithMode int8 allocates i8 storage" {
    const allocator = std.testing.allocator;
...
test "BatchedKVCache: appendKV int8 quantizes data" {
    const allocator = std.testing.allocator;
    const n_kv_heads = 1;
    const head_dim = 4;
    ...
    // Check i8 data was stored
    const k_i8 = cache.getKHeadI8(slot, 0);
    const v_i8 = cache.getVHeadI8(slot, 0);

    // With scale = 1.27/127 = 0.01, values should be:
    // k: 100, -50, 0, 127
    // v: -127, 50, -25, 75
    try std.testing.expectEqual(@as(i8, 100), k_i8[0]);
    try std.testing.expectEqual(@as(i8, 127), k_i8[3]);
    try std.testing.expectEqual(@as(i8, -127), v_i8[0]);
}

test "BatchedKVCache: quantize and dequantize roundtrip" {
    const allocator = std.testing.allocator;
    const n_kv_heads = 2;
    const head_dim = 4;
    var cache = try BatchedKVCache.initWithMode(allocator, 1, n_kv_heads, head_dim, 16, .int8);
    ...
    // Dequantize and check roundtrip error
    for (0..n_kv_heads) |kv_head| {
        const k_i8 = cache.getKHeadI8(slot, kv_head);
        const v_i8 = cache.getVHeadI8(slot, kv_head);
        const k_scale = cache.getKScales(slot, kv_head)[0];
        const v_scale = cache.getVScales(slot, kv_head)[0];
        ...
        for (0..head_dim) |i| {
            ...
            // INT8 quantization error should be < 1% of max value
            try std.testing.expect(error_k < 0.05);

            const orig_v = v_data[kv_head * head_dim + i];
            const dequant_v = @as(f32, @floatFromInt(v_i8[i])) * v_scale;
            const error_v = @abs(orig_v - dequant_v);
            try std.testing.expect(error_v < 0.05);
        }
    }
}

test "QuantMode.fromEnv parses environment" {
    // Can't easily test environment variables, but test the default behavior
    // When TALU_KV_QUANT is not set, should return .f32
    const mode = QuantMode.fromEnv();
    // This will be .f32 unless the test runner sets the env var
    try std.testing.expect(mode == .f32 or mode == .int8);
}
