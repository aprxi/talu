...
pub const CpuKernel = kernel_wrapper.CpuKernel;

pub const NUM_TMP_BUFFERS = scratch_mod.NUM_TMP_BUFFERS;
pub const ScratchBuffer = scratch_mod.ScratchBuffer;
...
fn fuseGateUpWeights(allocator: std.mem.Allocator, w1: *const Tensor, w3: *const Tensor) !?Tensor {
    ...
    // Skip fusion for quantized types - they have complex layouts with scales/biases
    if (dtype.isQuantized() or elem_size == 0) return null;

    const rows: usize = @intCast(w1.shape[0]);
    const cols: usize = @intCast(w1.shape[1]);

    // Determine fusion strategy based on dtype
    // F32: matmul expects [in, out] = [d_model, d_ff], so fuse columns -> [d_model, 2*d_ff]
    // BF16/F16: matmul expects [out, in] = [d_ff, d_model], so fuse rows -> [2*d_ff, d_model]
    const fuse_columns = (dtype == .f32);

    if (fuse_columns) {
        // F32: fuse along columns - [rows, 2*cols]
        ...
        // BF16/F16: fuse along rows - [2*rows, cols]
        ...
        // Copy w1 to first half (rows 0..rows)
        @memcpy(fused_data[0..tensor_size], w1_bytes[0..tensor_size]);
        // Copy w3 to second half (rows rows..2*rows)
...
pub fn blockWeightsFromMap(
    ...
    switch (block_type) {
        .attention_mlp => {
            ...
            return BlockWeights{ .attention_mlp = .{
...
fn addInto(a: *const Tensor, b: *const Tensor, out: *Tensor) void {
    ...
    while (elem_idx + VEC - 1 < numel) : (elem_idx += VEC) {
        const va: @Vector(VEC, f32) = a_data[elem_idx..][0..VEC].*;
        const vb: @Vector(VEC, f32) = b_data[elem_idx..][0..VEC].*;
        o_data[elem_idx..][0..VEC].* = va + vb;
    }
    // Scalar remainder
    while (elem_idx < numel) : (elem_idx += 1) {
        o_data[elem_idx] = a_data[elem_idx] + b_data[elem_idx];
    }
}
...
pub fn addIntoScaled(a: *const Tensor, b: *const Tensor, out: *Tensor, scale: f32) void {
    ...
    while (elem_idx + VEC - 1 < numel) : (elem_idx += VEC) {
        const va: @Vector(VEC, f32) = a_data[elem_idx..][0..VEC].*;
        const vb: @Vector(VEC, f32) = b_data[elem_idx..][0..VEC].*;
        o_data[elem_idx..][0..VEC].* = va + vb * vscale;
    }
    while (elem_idx < numel) : (elem_idx += 1) {
        o_data[elem_idx] = a_data[elem_idx] + b_data[elem_idx] * scale;
    }
}
