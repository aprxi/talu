...
const embedding = compute.cpu.embedding;
...
pub fn buildBlocks(
    ...
    for (block_array, block_weights, 0..) |*block_slot, block_weight, layer_idx| {
        ...
        var layer_head_dim: usize = @intCast(config.head_dim);
        var layer_attention_scale = base_attention_scale;
