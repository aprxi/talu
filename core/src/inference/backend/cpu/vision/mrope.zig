...
//!
//! This module centralizes mRoPE position/delta logic so CPU and Metal backends
//! stay behaviorally aligned while keeping backend-specific table fill code local.

const std = @import("std");
const tensor = @import("../../../../tensor.zig");
const vision_common = @import("types.zig");
...
test "applyPositionDelta rejects negative resulting positions" {
    try std.testing.expectError(error.InvalidShape, applyPositionDelta(2, -3));
    try std.testing.expectEqual(@as(usize, 7), try applyPositionDelta(4, 3));
}

test "resolveMropeSection accepts valid split and rejects invalid split" {
    var cfg = std.mem.zeroes(tensor.ModelConfig);
    cfg.rope_scaling.mrope_section = .{ 4, 2, 2 };
    try std.testing.expectEqualSlices(usize, &.{ 4, 2, 2 }, &resolveMropeSection(&cfg, 16));

    cfg.rope_scaling.mrope_section = .{ 8, 0, 0 };
    try std.testing.expectEqualSlices(usize, &.{ 0, 0, 0 }, &resolveMropeSection(&cfg, 16));
}

test "buildMultimodalMropePositions maps text and image spans" {
    const image_token_id: u32 = 99;
    const tokens = [_]u32{ 10, 11, image_token_id, 12 };
    ...
    var pos_t: [4]u32 = undefined;
    var pos_h: [4]u32 = undefined;
    var pos_w: [4]u32 = undefined;
    ...
    try std.testing.expectEqual(@as(u32, 0), pos_t[0]);
    try std.testing.expectEqual(@as(u32, 1), pos_t[1]);
    try std.testing.expectEqual(@as(u32, 2), pos_t[2]);
    try std.testing.expectEqual(@as(u32, 3), pos_t[3]);
    const delta = try computePositionDelta(&pos_t, &pos_h, &pos_w);
    try std.testing.expectEqual(@as(isize, 0), delta);
}
