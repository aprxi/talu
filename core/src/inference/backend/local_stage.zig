...
//! stage chain. Backends provide concrete stage executors; the bridge contracts
...
pub fn boundaryRowByteCount(d_model: usize, boundary_dtype: bridge.BoundaryDType) !u64 {
...
pub fn buildContractBundle(request: ContractBuildRequest) !ContractBundle {
    ...
    bundle.tensor_frame_plan_ref = try bridge.TensorFramePlanRef.fromStagePlan(request.allocator, plan_ptr);
    ...
    if (countStatefulDependencies(plan_ptr) > 0) {
        ...
        if (bundle.state_ownership_plan) |*state_plan| {
            bundle.state_placement_ref = try bridge.buildStageStatePlacementRef(request.allocator, state_plan);
        }
    }
    ...
    bundle.local_stage_runner_plan_ref = try bridge.buildLocalStageRunnerPlanRef(request.allocator, .{
        .stage_plan = plan_ptr,
...
fn buildBoundaryProfiles(
    ...
    const profiles = try allocator.alloc(bridge.BoundaryFrameProfile, profile_count);
...
fn buildHostCapabilities(
    ...
    const host_capabilities = try allocator.alloc(bridge.HostCapability, plan.stages.len);
    ...
    for (plan.stages, stage_backend_kinds, 0..) |stage, backend_kind, index| {
        ...
        const frames = try allocator.alloc(bridge.HostFrameCapability, frame_count);
        ...
        host_capabilities[index] = try bridge.buildHostCapability(allocator, .{
...
fn buildHostResidencies(
    ...
    const residencies = try allocator.alloc(bridge.HostResidencySnapshot, plan.stages.len);
    var initialized: usize = 0;
    errdefer {
    ...
    for (plan.stages, 0..) |stage, index| {
        if (stage.id != index) return error.InvalidTopologyConfig;
        const resident = [_]bridge.ResidentStageEntry{residentEntryFromStage(stage, state_ref)};
        residencies[index] = try bridge.buildHostResidencySnapshot(allocator, .{
            .host_id = try deterministicHostId(stage.id),
            .plan = plan,
...
fn buildStageHostBindings(
    ...
    const bindings = try allocator.alloc(bridge.StageHostBinding, plan.stages.len);
...
pub fn buildStateOwnershipPlan(
    ...
    const descriptor_sets = try allocator.alloc(bridge.StageStateDescriptorSet, plan.stages.len);
    ...
    const facts = try allocator.alloc(bridge.StageStatePartitionFact, fact_count);
    ...
    return bridge.buildStageStateOwnershipPlan(allocator, .{
        .plan = plan,
...
test "boundaryProfilePair emits prefill and decode profiles" {
    ...
    try std.testing.expectEqual(bridge.TensorFrameStepKind.prefill, pair[0].step_kind);
    try std.testing.expectEqual(@as(u64, 5 * 8 * @sizeOf(f32)), pair[0].max_activation_payload_bytes);
    try std.testing.expectEqual(bridge.TensorFrameStepKind.decode, pair[1].step_kind);
