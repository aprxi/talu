//! Response C API shared types.
//!
//! Defines C-compatible generation configuration types and converts them into
//! response generation options. The batch C API owns local generation execution.
//!
...
pub const CGenerateConfig = extern struct {
    ...
    /// Mapped to template variables (e.g. enable_thinking) by response serving.
