...
const session_schema = @import("schema.zig");
...
pub fn scanSessionsFiltered(
    ...
    // Session-ID string dedup: catches cross-hash duplicates during SipHash→Wyhash transition.
    // Same session_id may appear with different scalar hashes in old vs new blocks.
    ...
    if (fs_reader.current_path) |current_path| {
        ...
        if (file) |f| {
            ...
            while (idx > 0) {
                ...
                limit_reached.* = try processBlock(f, reader, entry.block_off, alloc, effective_params, &seen, &seen_ids, &results, &scanned);
    ...
    if (fs_reader.manifest_data) |manifest| {
        ...
        while (seg_idx > 0) {
            ...
            while (blk_idx > 0) {
                ...
                limit_reached.* = try processBlock(file, reader, entry.block_off, alloc, effective_params, &seen, &seen_ids, &results, &scanned);
                if (limit_reached.*) break;
            }
    ...
    for (new_indices.items) |idx| {
        ...
        if (record.marker) |marker| {
            if (std.mem.eql(u8, marker, "deleted")) {
                // Track tombstoned session_id so legacy-hash rows don't resurface.
                const gop_id = try seen_ids.getOrPut(record.session_id);
                if (!gop_id.found_existing) {
                    gop_id.key_ptr.* = try alloc.dupe(u8, record.session_id);
                }
                codec.freeScannedSessionRecord(alloc, &record);
        ...
        // 6b. Session ID dedup: catches cross-hash duplicates during SipHash→Wyhash transition.
        {
            const gop_id = try seen_ids.getOrPut(record.session_id);
            if (gop_id.found_existing) {
                codec.freeScannedSessionRecord(alloc, &record);
