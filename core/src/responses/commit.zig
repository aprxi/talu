...
const reasoning_parser_mod = conversation_mod.reasoning_parser;
...
pub fn commitGenerationResult(
    ...
    if (params.tool_calls.len > 0) {
        log.debug("responses", "Commit: tool calls", .{
            .count = params.tool_calls.len,
            .prompt_tokens = params.prompt_tokens,
            .completion_tokens = params.completion_tokens,
        }, @src());
        for (params.tool_calls) |tc| {
            log.trace("responses", "Tool call committed", .{
    ...
    log.debug("responses", "Commit: text", .{
...
test "commitGenerationResult: plain text creates AssistantMessage" {
    ...
    try std.testing.expectEqual(conversation_mod.MessageRole.assistant, msg.role);
