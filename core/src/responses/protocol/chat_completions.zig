...
//! The conversation module (`core/src/responses/conversation/`) should be pure
//! item-based state.
//!
...
const MessageData = conversation_mod.MessageData;
    ...
    while (i < conv.items_list.items.len) : (i += 1) {
        ...
        switch (item.data.getType()) {
            ...
            .reasoning => {
                ...
                //
                // For Chat Completions projection, we ONLY emit summary
            ...
            .unknown => {
                // Skip unknown items in Chat Completions format.
            },
        }
    }
...
///
/// This performs the "Unfolding" transform from Chat Completions format to Item-based format.
...
pub fn parse(
    ...
    for (array.items) |item_value| {
        ...
        const role: conversation_mod.MessageRole = if (std.mem.eql(u8, role_str, "system"))
            .system
        ...
        // Get content - supports both string and array content formats.
        const content_value = obj.get("content");
        if (content_value) |cv| switch (cv) {
            .string => |s| {
                // String content creates a text message.
                switch (role) {
