...
//!   - **Chat Completions Projection:** Folding transform for message-array endpoints
//!
//! # Schema Mapping
//!
...
pub const MessageRole = enum(u8) {
    ...
    pub fn toChatCompletionsRole(self: MessageRole) MessageRole {
        return switch (self) {
            .developer => .system,
            .unknown => .user,
            else => self,
...
pub const ContentPart = struct {
    ...
    /// Get the primary text/data field as a slice.
...
test "MessageRole.toChatCompletionsRole" {
    try std.testing.expectEqual(MessageRole.system, MessageRole.system.toChatCompletionsRole());
    try std.testing.expectEqual(MessageRole.user, MessageRole.user.toChatCompletionsRole());
    try std.testing.expectEqual(MessageRole.assistant, MessageRole.assistant.toChatCompletionsRole());
    try std.testing.expectEqual(MessageRole.system, MessageRole.developer.toChatCompletionsRole());
    // Unknown roles map to user for Chat Completions projection.
    try std.testing.expectEqual(MessageRole.user, MessageRole.unknown.toChatCompletionsRole());
}
