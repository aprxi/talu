...
const collab_memory = @import("../collab/memory.zig");
...
fn validateRequiredArg(arg: ?[*:0]const u8, name: []const u8) ?[]const u8 {
    const value = arg orelse {
        capi_error.setErrorWithCode(.invalid_argument, "{s} is required", .{name});
        return null;
    };
    const slice = std.mem.span(value);
    if (slice.len == 0) {
        capi_error.setErrorWithCode(.invalid_argument, "{s} must not be empty", .{name});
        return null;
    }
    return slice;
}

fn optSlice(arg: ?[*:0]const u8) ?[]const u8 {
    return if (arg) |value| std.mem.span(value) else null;
}
...
fn buildWatchBatch(events: []const collab_memory.WatchEvent, lost: bool) !CCollabWatchBatch {
    const arena_ptr = try buildArena();
    errdefer {
        arena_ptr.deinit();
        allocator.destroy(arena_ptr);
...
pub export fn talu_collab_init(
    ...
    // TODO: a persistent collab backend should bind to TALU_DB_HOST.
    // For now `db_path` is accepted for API stability but ignored because the
    // collab runtime is intentionally process-local and in-memory only.
    _ = if (db_path) |value| std.mem.span(value) else null;

    const kind = validateRequiredArg(resource_kind, "resource_kind") orelse {
        return @intFromEnum(error_codes.ErrorCode.invalid_argument);
    };
    const id = validateRequiredArg(resource_id, "resource_id") orelse {
        return @intFromEnum(error_codes.ErrorCode.invalid_argument);
    };
    ...
    store_ptr.* = collab_resource.ResourceStore.init(allocator, "", kind, id) catch |err| {
...
pub export fn talu_collab_open_session(
    ...
    const pid = validateRequiredArg(participant_id, "participant_id") orelse {
        return @intFromEnum(error_codes.ErrorCode.invalid_argument);
    };
    ...
    const session = store.openSession(pid, parsed_kind, optSlice(role)) catch |err| {
...
pub export fn talu_collab_submit_op(
    ...
    const actor = validateRequiredArg(actor_id, "actor_id") orelse {
        return @intFromEnum(error_codes.ErrorCode.invalid_argument);
    };
    const op = validateRequiredArg(op_id, "op_id") orelse {
        return @intFromEnum(error_codes.ErrorCode.invalid_argument);
    };
...
pub export fn talu_collab_clear_snapshot(
    ...
    const op = validateRequiredArg(op_kind, "op_kind") orelse {
        return @intFromEnum(error_codes.ErrorCode.invalid_argument);
    };
    const out = out_result orelse {
        capi_error.setErrorWithCode(.invalid_argument, "out_result is null", .{});
