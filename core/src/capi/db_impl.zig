...
pub fn talu_vector_store_free(handle: ?*VectorStoreHandle) callconv(.c) void {
    capi_error.clearError();
...
/// the handle pointer.
pub fn talu_vector_store_simulate_crash(handle: ?*VectorStoreHandle) callconv(.c) void {
    capi_error.clearError();
    const backend: *db.vector.store.VectorAdapter = @ptrCast(@alignCast(handle orelse return));
...
pub fn talu_vector_store_append_ex(
    handle: ?*VectorStoreHandle,
    ids_ptr: ?[*]const u64,
    vectors_ptr: ?[*]const f32,
    count: usize,
...
pub fn talu_vector_store_append_idempotent_ex(
    handle: ?*VectorStoreHandle,
    ids_ptr: ?[*]const u64,
    vectors_ptr: ?[*]const f32,
    count: usize,
...
pub fn talu_vector_store_upsert(
    handle: ?*VectorStoreHandle,
    ids_ptr: ?[*]const u64,
    vectors_ptr: ?[*]const f32,
    count: usize,
...
pub fn talu_vector_store_upsert_ex(
    handle: ?*VectorStoreHandle,
    ids_ptr: ?[*]const u64,
    vectors_ptr: ?[*]const f32,
    count: usize,
...
pub fn talu_vector_store_upsert_idempotent_ex(
    handle: ?*VectorStoreHandle,
    ids_ptr: ?[*]const u64,
    vectors_ptr: ?[*]const f32,
    count: usize,
...
pub fn talu_vector_store_delete(
    handle: ?*VectorStoreHandle,
    ids_ptr: ?[*]const u64,
    count: usize,
...
pub fn talu_vector_store_delete_idempotent(
    handle: ?*VectorStoreHandle,
    ids_ptr: ?[*]const u64,
    count: usize,
    key_hash: u64,
...
pub fn talu_vector_store_fetch(
    handle: ?*VectorStoreHandle,
    ids_ptr: ?[*]const u64,
    count: usize,
    include_values: bool,
...
pub fn talu_vector_store_stats(
    handle: ?*VectorStoreHandle,
...
pub fn talu_vector_store_compact_idempotent(
    handle: ?*VectorStoreHandle,
    dims: u32,
    key_hash: u64,
    request_hash: u64,
...
pub fn talu_vector_store_compact_with_generation(
    handle: ?*VectorStoreHandle,
    dims: u32,
...
pub fn talu_vector_store_changes(
    handle: ?*VectorStoreHandle,
    since: u64,
    limit: usize,
    out_seqs: *?[*]u64,
...
pub fn talu_vector_store_search_batch_ex(
    handle: ?*VectorStoreHandle,
    query_ptr: ?[*]const f32,
    query_len: usize,
    dims: u32,
...
pub fn talu_vector_store_free_fetch(
    ids_ptr: ?[*]u64,
    vectors_ptr: ?[*]f32,
    found_count: usize,
    dims: u32,
...
pub fn talu_vector_store_free_changes(
    seqs_ptr: ?[*]u64,
    ops_ptr: ?[*]u8,
    ids_ptr: ?[*]u64,
    timestamps_ptr: ?[*]i64,
...
pub fn talu_storage_list_sessions_batch(
    db_path: ?[*:0]const u8,
    offset: u32,
    limit: u32,
    group_id: ?[*:0]const u8,
...
///   - storage_error if session not found
///   - resource_busy if database is locked by another process
pub fn talu_storage_delete_session(
    db_path: ?[*:0]const u8,
    session_id: ?[*:0]const u8,
) callconv(.c) i32 {
    capi_error.clearError();
...
pub fn talu_storage_free_sessions(
...
///
/// Returns: 0 on success, negative error code on failure.
pub fn talu_chat_set_max_segment_size(
    chat_handle: ?*ChatHandle,
    max_bytes: u64,
) callconv(.c) i32 {
    capi_error.clearError();
...
///
/// Returns: 0 on success, negative error code on failure.
pub fn talu_chat_set_durability(
    chat_handle: ?*ChatHandle,
    mode: u8,
) callconv(.c) i32 {
    capi_error.clearError();
...
///
/// Returns: 0 on success, negative error code on failure.
pub fn talu_vector_store_set_durability(
    handle: ?*VectorStoreHandle,
    mode: u8,
) callconv(.c) i32 {
    capi_error.clearError();
...
///
/// Returns: 0 on success, negative error code on failure.
pub fn talu_chat_simulate_crash(
    chat_handle: ?*ChatHandle,
) callconv(.c) i32 {
    capi_error.clearError();
...
///
/// Returns: 0 on success, negative error code on failure.
pub fn talu_storage_delete_tag(
    db_path: ?[*:0]const u8,
    tag_id: ?[*:0]const u8,
) callconv(.c) i32 {
    capi_error.clearError();
...
pub fn talu_storage_free_string_list(
    list: ?*CStringList,
) callconv(.c) void {
    capi_error.clearError();
