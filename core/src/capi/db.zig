...
pub export fn talu_vector_store_upsert(
    ...
    const total = count * @as(usize, dims);
    backend.upsertBatch(ids[0..count], vectors[0..total], dims) catch |err| {
        capi_error.setError(err, "failed to upsert vectors", .{});
        return @intFromEnum(error_codes.errorToCode(err));
    };
    return 0;
}
...
pub export fn talu_vector_store_delete(
    ...
    const result = backend.deleteIds(ids[0..count]) catch |err| {
        capi_error.setError(err, "failed to delete vectors", .{});
        return @intFromEnum(error_codes.errorToCode(err));
    };
...
pub export fn talu_vector_store_compact(
    ...
    const result = backend.compact(dims) catch |err| {
        capi_error.setError(err, "failed to compact vectors", .{});
        return @intFromEnum(error_codes.errorToCode(err));
    };
