//! C API boundary for training session.
//!
...
test "capi_boundary createSession and destroySession" {
...
test "capi_boundary configure roundtrip" {
    const allocator = std.testing.allocator;
    const session = try createSession(allocator);
    defer destroySession(allocator, session);
...
test "capi_boundary callback wrapper" {
    const allocator = std.testing.allocator;
    const session = try createSession(allocator);
    defer destroySession(allocator, session);
