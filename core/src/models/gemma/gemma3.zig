...
/// Gemma3 architecture includes extra norm steps on attention and MLP branches:
...
const gemma3_block_weights = [_]types.WeightSpec{
    ...
    .{ .id = "post_feedforward_layernorm.weight", .suffix = "post_feedforward_layernorm.weight", .module_type = "RMSNorm", .layout = .none, .dtype = "float32", .required = true },
