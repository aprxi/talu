...
pub fn computeFlashDecodeGQA_I8(
    ...
    // Step 4: scores @ V (V is already f32, no dequant needed)
    scoresTimesV(scores, active_v, output_rows[0 .. n_queries * feature_width], n_queries, feature_width, active_len);
}
...
// =============================================================================
// Platform-optimized scores @ V matmul
// =============================================================================

/// Compute C = scores @ V where scores is [m, k] and V is [k, n], C is [m, n].
/// On macOS uses Accelerate BLAS (AMX/NEON). On x86 uses SIMD weighted accumulation.
fn scoresTimesV(scores: []const f32, v: []const f32, output: []f32, m: usize, n: usize, k: usize) void {
    if (comptime accelerate.available) {
        accelerate.sgemm(scores, v, output, m, n, k);
        return;
    }
    for (0..m) |row| {
        const row_scores = scores[row * k ..][0..k];
        const out = output[row * n ..][0..n];
        @memset(out, 0);
        for (0..k) |v_idx| {
            reduction.weightedAccumulateRow(out, v[v_idx * n ..][0..n], row_scores[v_idx]);
        }
    }
}
...
pub fn computeMaskedRowWeightedSumI8(
    ...
    // scores @ V (V is already f32, no dequant needed)
    const active_len = history_len - valid_start_index;
