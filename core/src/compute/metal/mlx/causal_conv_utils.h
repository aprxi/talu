...
inline array causal_conv_run_sequence(
    ...
    const int batch = bx.shape(0);
    if (batch <= 0) {
        throw std::invalid_argument("[causal_conv] batch must be > 0");
    }
    if (conv_state->shape(0) != batch || conv_state->shape(1) != d_conv_i || conv_state->shape(2) != conv_dim_i) {
        throw std::invalid_argument("[causal_conv] conv_state shape mismatch");
    }

    // Decode fast path (seq_len == 1): avoid a full conv1d launch and compute
    // the same depthwise update with direct tensor ops.
    if (seq_len == 1) {
        const array shifted = slice(*conv_state, {0, 1, 0}, {batch, d_conv_i, conv_dim_i});
        ...
        *conv_state = next_state;
        return reshape(conv_token, {batch, 1, conv_dim_i});
    }
    ...
    const array conv_seq = slice(conv_full, {0, 1, 0}, {batch, seq_len + 1, conv_dim_i});

    // Persist rolling history for next call.
    *conv_state = slice(history, {0, seq_len, 0}, {batch, seq_len + d_conv_i, conv_dim_i});
    return conv_seq;
}
...
inline array causal_conv_run_sequence_stepwise(
    ...
    for (int token_idx = 0; token_idx < seq_len; ++token_idx) {
        const array bx_token = slice(bx, {0, token_idx, 0}, {batch, token_idx + 1, conv_dim_i});
        const array shifted = slice(state, {0, 1, 0}, {batch, d_conv_i, conv_dim_i});
        state = concatenate({shifted, bx_token}, 1);
        const array conv_token = sum(state * conv_kernel_broadcast, 1);
        outputs.emplace_back(reshape(conv_token, {batch, 1, conv_dim_i}));
    }
