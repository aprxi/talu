...
/// ShortConv mixer (dense projections): in_proj -> causal depthwise conv -> out_proj
pub extern fn mlx_lazy_shortconv_mixer_bf16(
    input: ArrayHandle,
    in_proj: ArrayHandle,
    conv_weight: ArrayHandle,
    conv_bias: ArrayHandle, // can be null
    out_proj: ArrayHandle,
    shortconv_cache: ShortConvCacheHandle, // can be null
    layer_idx: usize,
    d_conv: usize,
    conv_dim: usize,
) ArrayHandle;

/// ShortConv mixer (quantized in/out projections): in_proj -> causal depthwise conv -> out_proj
...
/// Fused mixed attention: quantized Q/K/V projections + dense BF16/F16/F32 output projection.
...
pub extern fn mlx_lazy_fused_ffn(
    ...
    group_size: usize,
    bits: usize,
    use_gelu: bool, // GELU activation instead of SiLU
) ArrayHandle;
...
/// For MoE models (e.g. 32 experts per layer, 4 active per token)
