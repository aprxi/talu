//! Row-strided fused residual-add + output-scale kernel wrapper for CUDA runtime.
...
pub const embedded_symbol: [:0]const u8 = "talu_residual_add_scaled_rows_strided_f32";
pub const op_name: []const u8 = "residual_add_scaled_rows_strided_f32";
...
pub fn runWithFunction(
    ...
    try arg_pack.appendScalar(f32, output_scale);
...
test "validateArgs rejects stride smaller than cols" {
    ...
    try std.testing.expectError(
        ...
        validateArgs(&fake, &fake, &out, 2, 64, 64, 32, 64),
    );
}
