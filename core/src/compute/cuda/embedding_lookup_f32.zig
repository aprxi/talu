//! F32 embedding lookup kernel wrapper for modular CUDA runtime.
...
pub const embedded_symbol: [:0]const u8 = "talu_embedding_lookup_f32";
pub const op_name: []const u8 = "embedding_lookup_f32";
...
pub fn run(
    ...
    try validateArgs(out, embeddings, dim0, dim1, hidden_dim, token, layout_tag);
...
fn validateArgs(
    ...
    const embed_bytes = std.math.mul(usize, embed_count, @sizeOf(f32)) catch return error.InvalidArgument;
...
test "run rejects invalid layout tag" {
    ...
    const embeddings = device_mod.Buffer{ .pointer = 0, .size = 16 };
