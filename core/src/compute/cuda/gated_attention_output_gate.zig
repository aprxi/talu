//! Query-gated attention output gating kernel wrapper.
...
pub const embedded_symbol: [:0]const u8 = "talu_gated_attention_output_gate_f32";
pub const op_name: []const u8 = "gated_attention_output_gate_f32";

pub fn runWithFunction(
    arg_pack: *args_mod.ArgPack,
    device: *device_mod.Device,
    function: module_mod.Function,
    context: *device_mod.Buffer,
    ...
    try validateArgs(context, packed_query, seq_len, query_dim, query_projection_dim, head_count, head_dim);

    arg_pack.reset();
    try arg_pack.appendBufferPtr(context);
...
fn validateArgs(
    context: *const device_mod.Buffer,
    packed_query: *const device_mod.Buffer,
    seq_len: u32,
    query_dim: u32,
    query_projection_dim: u32,
    ...
    const context_bytes = std.math.mul(
        usize,
    ...
    if (context.size < context_bytes or packed_query.size < packed_bytes) {
        return error.InvalidArgument;
    }
}
...
test "validateArgs rejects undersized buffers" {
    const context = device_mod.Buffer{ .pointer = 0, .size = 2 * @sizeOf(f32) };
    const packed_buf = device_mod.Buffer{ .pointer = 0, .size = 4 * @sizeOf(f32) };
    try std.testing.expectError(
        error.InvalidArgument,
        validateArgs(&context, &packed_buf, 2, 4, 8, 2, 2),
    );
}
