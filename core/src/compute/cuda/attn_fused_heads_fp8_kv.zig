//! Fused attention kernel wrapper for fp8 KV cache.
...
pub const embedded_symbol: [:0]const u8 = "talu_attn_fused_heads_fp8_kv";
pub const op_name: []const u8 = "attn_fused_heads_fp8_kv";
...
fn validateArgs(
    ...
    if (n_heads == 0 or n_kv_heads == 0 or seq_len == 0 or row_stride == 0 or kv_groups == 0 or head_dim == 0) return error.InvalidArgument;
...
test "validateArgs rejects invalid shape params" {
    ...
    try std.testing.expectError(error.InvalidArgument, validateArgs(&query, &cache, &cache, &scales, &scales, &out, 0, 0, 1, 1, 1, 1, 1.0, 1, 10000.0));
    try std.testing.expectError(error.InvalidArgument, validateArgs(&query, &cache, &cache, &scales, &scales, &out, 8, 2, 2, 4, 3, 2, 1.0, 2, 10000.0));
}
