//! Fused causal prefill attention kernel wrapper for f16 KV cache.
...
pub const embedded_symbol: [:0]const u8 = "talu_attn_fused_prefill_heads_f16_kv";
pub const op_name: []const u8 = "attn_fused_prefill_heads_f16_kv";
...
test "validateArgs rejects invalid shape params" {
    ...
    try std.testing.expectError(error.InvalidArgument, validateArgs(&query, &cache, &cache, &out, 0, 1, 1, 1, 1, 1, 1.0, 1, 0, 10000.0));
    try std.testing.expectError(error.InvalidArgument, validateArgs(&query, &cache, &cache, &out, 8, 3, 2, 4, 2, 2, 1.0, 2, 0, 10000.0));
}
