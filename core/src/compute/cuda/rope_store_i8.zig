//! RoPE + i8 cache-store kernel wrapper.
//! Per-head symmetric quantization with scale = max_abs / 127.
...
pub const embedded_symbol: [:0]const u8 = "talu_rope_store_i8";
pub const op_name: []const u8 = "rope_store_i8";
...
pub fn runWithFunction(
    ...
    try validateArgs(input_f32, out_i8, scales, n_heads, head_dim, rope_dim, theta);

    arg_pack.reset();
    try arg_pack.appendBufferPtr(out_i8);
...
fn validateArgs(
    ...
    if (input_f32.size < input_bytes or out_i8.size < out_bytes or scales.size < scale_bytes) return error.InvalidArgument;
}
