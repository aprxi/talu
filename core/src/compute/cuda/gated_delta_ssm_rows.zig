//! Gated-delta state-space rows kernel wrapper.
...
pub fn runWithFunction(
    ...
    try validateArgs(qkv_rows, a_log, dt_bias, state, out, n_qk_heads, n_v_heads, d_head, rows, row_stride, beta_offset, a_offset, out_row_stride);
...
fn validateArgs(
    ...
    if (state.size < state_bytes) return error.InvalidArgument;
}
