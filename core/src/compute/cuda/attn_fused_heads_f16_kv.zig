...
pub fn runWithFunction(
    ...
    try validateArgs(query, key_cache_f16, value_cache_f16, out, n_heads, seq_len, row_stride, kv_groups, head_dim, scale);
...
test "validateArgs rejects invalid shape params" {
    ...
    try std.testing.expectError(error.InvalidArgument, validateArgs(&query, &cache, &cache, &out, 0, 1, 1, 1, 1, 1.0));
    try std.testing.expectError(error.InvalidArgument, validateArgs(&query, &cache, &cache, &out, 8, 2, 4, 3, 2, 1.0));
}
