//! GQA-aware fused causal prefill attention kernel wrapper for fp8 KV cache.
//!
//! Groups Q heads sharing a KV head into one block, loads KV tiles into shared
//! memory once, and serves all warps from smem.  Reduces global memory
//! bandwidth by kv_groups× compared to the non-GQA kernel.
//!
//! Grid: (n_kv_heads, q_rows).  Block: kv_groups * 32 threads.
//! Dynamic shared memory: 2 * gqa_kv_tile * head_dim * 1 (fp8) + 2 * gqa_kv_tile * sizeof(f32) (scales).
...
pub const embedded_symbol: [:0]const u8 = "talu_attn_fused_prefill_heads_fp8_kv_gqa";
pub const op_name: []const u8 = "attn_fused_prefill_heads_fp8_kv_gqa";

/// Must match GQA_FP8_KV_TILE in attention_ops.cu.
const gqa_fp8_kv_tile: u32 = 32;
...
pub fn runWithFunction(
    ...
    key_cache_fp8: *const device_mod.Buffer,
    value_cache_fp8: *const device_mod.Buffer,
    ...
    try arg_pack.appendBufferPtr(key_cache_fp8);
    try arg_pack.appendBufferPtr(value_cache_fp8);
    ...
    const block_threads = kv_groups * 32;
    // Shared memory: fp8 KV tiles + f32 scale arrays.
    // k_smem[tile * head_dim] + v_smem[tile * head_dim] (fp8) +
    // k_sc_smem[tile] + v_sc_smem[tile] (f32)
    const smem_kv: u32 = 2 * gqa_fp8_kv_tile * head_dim; // fp8 bytes
    const smem_scales: u32 = 2 * gqa_fp8_kv_tile * @sizeOf(f32); // f32 scale bytes
    const shared_mem_bytes: u32 = smem_kv + smem_scales;
...
fn validateArgs(
    ...
    if (kv_groups < 2) return error.InvalidArgument;
    ...
    const block_threads = kv_groups * 32;
    if (block_threads > 1024) return error.InvalidArgument;
    ...
    if (query.size < query_bytes or key_cache_fp8.size < cache_elems or value_cache_fp8.size < cache_elems or out.size < out_bytes) {
        return error.InvalidArgument;
    }
    if (k_scales.size < scale_bytes or v_scales.size < scale_bytes) return error.InvalidArgument;
}

test "validateArgs rejects kv_groups < 2" {
    const query = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    const cache = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    const scales = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    var out = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    try std.testing.expectError(error.InvalidArgument, validateArgs(&query, &cache, &cache, &scales, &scales, &out, 8, 8, 4, 8, 64, 1, 64, 1.0, 64, 0, 10000.0));
}

test "validateArgs rejects block size > 1024" {
    const query = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    const cache = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    const scales = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    var out = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    try std.testing.expectError(error.InvalidArgument, validateArgs(&query, &cache, &cache, &scales, &scales, &out, 64, 1, 1, 1, 64, 64, 64, 1.0, 64, 0, 10000.0));
}

test "validateArgs accepts valid GQA params" {
    const query = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    const cache = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    const scales = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    var out = device_mod.Buffer{ .pointer = 0, .size = 65536 };
    try validateArgs(&query, &cache, &cache, &scales, &scales, &out, 8, 2, 2, 4, 16, 4, 8, 1.0, 8, 0, 10000.0);
}
