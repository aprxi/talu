//! Batched attention weighted-sum kernel wrapper for fp8 KV cache.
...
pub const embedded_symbol: [:0]const u8 = "talu_attn_weighted_sum_heads_fp8_kv";
pub const op_name: []const u8 = "attn_weighted_sum_heads_fp8_kv";
...
pub fn runWithFunction(
    ...
    try validateArgs(probs, value_cache_fp8, v_scales, out, n_heads, n_kv_heads, seq_len, row_stride, kv_groups, head_dim);
...
fn validateArgs(
    ...
    if (probs.size < prob_bytes or value_cache_fp8.size < cache_elems or out.size < out_bytes) {
        return error.InvalidArgument;
    }
    if (v_scales.size < scale_bytes) return error.InvalidArgument;
}
...
test "validateArgs rejects invalid shapes" {
    ...
    try std.testing.expectError(
        ...
        validateArgs(&probs, &cache, &scales, &out, 8, 2, 2, 4, 3, 2),
    );
    try std.testing.expectError(
        error.InvalidArgument,
        validateArgs(&probs, &cache, &scales, &out, 0, 0, 2, 4, 1, 2),
    );
}
