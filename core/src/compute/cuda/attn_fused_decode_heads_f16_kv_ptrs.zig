//! Batched fused decode-attention wrapper using per-row KV cache pointers.
//! Supports optional query gate fusion: when gate_proj is non-null, the kernel
//! reads sigmoid gate values from the packed projection and applies them to
//! the attention output, eliminating a separate gating kernel launch.
...
pub const embedded_symbol: [:0]const u8 = "talu_attn_fused_decode_heads_f16_kv_ptrs";
pub const op_name: []const u8 = "attn_fused_decode_heads_f16_kv_ptrs";
...
fn validateArgs(
    ...
    if (batch_rows == 0 or n_heads == 0 or row_stride == 0 or kv_groups == 0 or head_dim == 0) return error.InvalidArgument;
...
test "validateArgs rejects invalid dimensions" {
    ...
    try std.testing.expectError(
        ...
        validateArgs(&b, &b, &b, &b, &b, &b, 0, 8, 64, 1, 64, 1.0, 64, 10000.0, null, 0),
    );
    try std.testing.expectError(
        error.InvalidArgument,
        validateArgs(&b, &b, &b, &b, &b, &b, 1, 8, 64, 3, 64, 1.0, 64, 10000.0, null, 0),
    );
}
