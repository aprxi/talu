//! K/V cache write kernel wrapper (K: RoPE+store i8, V: cast+store i8).
...
pub const embedded_symbol: [:0]const u8 = "talu_kv_write_i8";
pub const op_name: []const u8 = "kv_write_i8";
...
fn validateArgs(
    ...
    const out_bytes = count; // 1 byte per i8 element
    ...
    if (out_k_i8.size < out_bytes or out_v_i8.size < out_bytes) return error.InvalidArgument;
