//! Vector add with scalar multiplier kernel wrapper for modular CUDA runtime.
...
pub const embedded_symbol: [:0]const u8 = "talu_vector_add_scaled_f32";
pub const op_name: []const u8 = "vector_add_scaled_f32";
...
pub fn run(
    ...
    try runWithFunction(&arg_pack, device, resolved.function, a, b, out, scale, count);
...
test "run rejects zero count" {
    ...
    try std.testing.expectError(
        ...
        run(std.testing.allocator, &fake_device, &registry, &fake_buffer, &fake_buffer, &out, 1.0, 0),
    );
}
