//! Batched K/V cache write kernel wrapper (K: RoPE+store i8, V: cast+store i8).
...
pub const embedded_symbol: [:0]const u8 = "talu_kv_write_i8_rows";
pub const op_name: []const u8 = "kv_write_i8_rows";
...
pub fn runWithFunction(
    ...
    out_k_i8: *device_mod.Buffer,
    out_v_i8: *device_mod.Buffer,
    ...
    arg_pack.reset();
    try arg_pack.appendBufferPtr(out_k_i8);
    try arg_pack.appendBufferPtr(out_v_i8);
...
fn validateArgs(
    ...
    // Output: i8 (1 byte per element), with row_stride spacing.
    ...
    if (out_k_i8.size < output_count or out_v_i8.size < output_count) return error.InvalidArgument;
