//! Batched K/V cache write kernel wrapper using per-row output pointers (i8).
//! Per-head-per-token symmetric quantization with scale = max_abs / 127.
...
pub const embedded_symbol: [:0]const u8 = "talu_kv_write_i8_rows_ptrs";
pub const op_name: []const u8 = "kv_write_i8_rows_ptrs";
...
fn validateArgs(
    ...
    if (out_k_ptrs.size < ptr_bytes or out_v_ptrs.size < ptr_bytes) return error.InvalidArgument;
    if (k_scale_ptrs.size < ptr_bytes or v_scale_ptrs.size < ptr_bytes) return error.InvalidArgument;
    if (positions.size < position_bytes) return error.InvalidArgument;
}
...
test "validateArgs rejects invalid row shape" {
    ...
    try std.testing.expectError(error.InvalidArgument, validateArgs(&input, &input, &ptrs, &ptrs, &ptrs, &ptrs, &positions, 2, 8, 8, 0, 16, 10000.0));
    try std.testing.expectError(error.InvalidArgument, validateArgs(&input, &input, &ptrs, &ptrs, &ptrs, &ptrs, &positions, 2, 8, 8, 2, 15, 10000.0));
}
