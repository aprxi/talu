//! Grouped-affine U4 weight dequantization to F16 kernel wrapper.
...
pub const embedded_symbol: [:0]const u8 = "talu_gaffine_u4_dequantize_to_f16";
pub const op_name: []const u8 = "gaffine_u4_dequantize_to_f16";
...
fn validateArgs(
    ...
    if ((in_dim % 16) != 0 or (group_size % 8) != 0) return error.InvalidArgument;
...
test "validateArgs rejects invalid alignment" {
    const b = device_mod.Buffer{ .pointer = 0, .size = 8192 };
    var out = b;
    // in_dim must be multiple of 16.
    try std.testing.expectError(error.InvalidArgument, validateArgs(&b, &b, &b, &out, 8, 12, 8, gaffine.scales_dtype_bf16));
    // group_size must be multiple of 8.
