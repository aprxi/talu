//! Batched attention-score kernel wrapper for fp8 KV cache.
...
pub const embedded_symbol: [:0]const u8 = "talu_attn_scores_heads_fp8_kv";
pub const op_name: []const u8 = "attn_scores_heads_fp8_kv";
...
pub fn runWithFunction(
    ...
    try validateArgs(query, key_cache_fp8, k_scales, scores_out, n_heads, n_kv_heads, seq_len, row_stride, kv_groups, head_dim, scale);
...
fn validateArgs(
    ...
    if (query.size < query_bytes or key_cache_fp8.size < cache_elems or scores_out.size < score_bytes) {
        return error.InvalidArgument;
    }
    if (k_scales.size < scale_bytes) return error.InvalidArgument;
}
...
test "validateArgs rejects invalid shapes" {
    ...
    try std.testing.expectError(
        ...
        validateArgs(&query, &cache, &scales, &scores, 0, 0, 1, 1, 1, 1, 1.0),
    );
    try std.testing.expectError(
        error.InvalidArgument,
        validateArgs(&query, &cache, &scales, &scores, 8, 2, 2, 4, 3, 2, 1.0),
    );
}
