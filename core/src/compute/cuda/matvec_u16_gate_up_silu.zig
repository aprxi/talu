...
pub const embedded_symbol_f16: [:0]const u8 = "talu_matvec_gate_up_silu_f16_f32";
pub const embedded_symbol_bf16: [:0]const u8 = "talu_matvec_gate_up_silu_bf16_f32";
...
pub fn runWithFunction(
    ...
    try validateArgs(input, gate_weight_u16, up_weight_u16, out, out_dim, in_dim);
...
test "validateArgs rejects zero dimensions" {
    ...
    var out = b;
    try std.testing.expectError(error.InvalidArgument, validateArgs(&b, &b, &b, &out, 0, 4));
    try std.testing.expectError(error.InvalidArgument, validateArgs(&b, &b, &b, &out, 4, 0));
}
...
test "validateArgs rejects undersized output" {
    ...
    try std.testing.expectError(error.InvalidArgument, validateArgs(&input, &weight, &weight, &out, 8, 8));
}
