//! Grouped-affine U8 matvec kernel wrapper for modular CUDA runtime.
...
pub const embedded_symbol: [:0]const u8 = "talu_gaffine_u8_matvec_f32";
pub const op_name: []const u8 = "gaffine_u8_matvec_f32";
...
test "validateArgs rejects invalid group alignment" {
    ...
    try std.testing.expectError(
        ...
        validateArgs(&b, &b, &b, &b, &out, 16, 8, 6, scales_dtype_bf16, 1),
    );
}
...
test "validateArgs rejects invalid scales dtype tag" {
    ...
    try std.testing.expectError(
        error.InvalidArgument,
        validateArgs(&b, &b, &b, &b, &out, 32, 8, 16, 7, 1),
    );
}
...
test "validateArgs rejects zero batch rows" {
    ...
    try std.testing.expectError(
        error.InvalidArgument,
        validateArgs(&b, &b, &b, &b, &out, 32, 8, 16, scales_dtype_bf16, 0),
    );
}
