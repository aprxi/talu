//! Dequantize fp8-e4m3 K/V cache rows to f16 using per-token-per-head scales.
...
pub const embedded_symbol: [:0]const u8 = "talu_dequant_kv_fp8_to_f16";
pub const op_name: []const u8 = "dequant_kv_fp8_to_f16";
...
fn validateArgs(
    ...
    const cache_bytes = cache_elems; // fp8 storage
    ...
    if (key_cache_fp8.size < cache_bytes or value_cache_fp8.size < cache_bytes) return error.InvalidArgument;
