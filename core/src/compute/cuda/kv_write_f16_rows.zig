//! Batched K/V cache write kernel wrapper (K: RoPE+store f16, V: cast+store f16).
...
pub const embedded_symbol: [:0]const u8 = "talu_kv_write_f16_rows";
pub const op_name: []const u8 = "kv_write_f16_rows";
...
fn validateArgs(
    ...
    const output_bytes = std.math.mul(usize, output_count, @sizeOf(u16)) catch return error.InvalidArgument;
    if (input_k_f32.size < input_bytes or input_v_f32.size < input_bytes) return error.InvalidArgument;
    if (out_k_f16.size < output_bytes or out_v_f16.size < output_bytes) return error.InvalidArgument;
}
...
test "validateArgs rejects invalid row shape" {
    ...
    try std.testing.expectError(error.InvalidArgument, validateArgs(&input, &input, &output, &output, 2, 8, 8, 0, 16, 10000.0));
    try std.testing.expectError(error.InvalidArgument, validateArgs(&input, &input, &output, &output, 2, 8, 8, 2, 15, 10000.0));
}
