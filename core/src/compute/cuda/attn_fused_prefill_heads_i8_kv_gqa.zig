//! GQA-aware fused causal prefill attention kernel wrapper for i8 KV cache.
//!
...
//! Dynamic shared memory: 2 * gqa_kv_tile * head_dim * 1 (i8) + 2 * gqa_kv_tile * sizeof(f32) (scales).
...
pub const embedded_symbol: [:0]const u8 = "talu_attn_fused_prefill_heads_i8_kv_gqa";
pub const op_name: []const u8 = "attn_fused_prefill_heads_i8_kv_gqa";

/// Must match GQA_I8_KV_TILE in attention_ops.cu.
const gqa_i8_kv_tile: u32 = 32;
...
pub fn runWithFunction(
    ...
    // Shared memory: i8 KV tiles + f32 scale arrays.
    // k_smem[tile * head_dim] + v_smem[tile * head_dim] (i8) +
    // k_sc_smem[tile] + v_sc_smem[tile] (f32)
    const smem_kv: u32 = 2 * gqa_i8_kv_tile * head_dim; // i8 bytes
    const smem_scales: u32 = 2 * gqa_i8_kv_tile * @sizeOf(f32); // f32 scale bytes
