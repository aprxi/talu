...
pub const extractTagsFromMetadata = codec.extractTagsFromMetadata;
...
pub const tagsContainWord = search.tagsContainWord;
pub const tagsMatchAll = search.tagsMatchAll;
pub const tagsMatchAny = search.tagsMatchAny;
...
pub const TableAdapter = struct {
    ...
    pub fn initReadOnly(alloc: Allocator, db_root: []const u8) !TableAdapter {
        const reader_ptr = try alloc.create(db_reader.Reader);
    ...
    fn writeItem(self: *TableAdapter, record: ItemRecord) !void {
        ...
        try self.fs_writer.appendRow(schema_items, &columns);
    }

    fn writeDelete(self: *TableAdapter, item_id: u64, deleted_at_ms: i64) !void {
        try self.ensureSchema(schema_deletes);
        ...
        try self.fs_writer.appendRow(schema_deletes, &columns);
    }
    ...
    fn writeSession(self: *TableAdapter, record: SessionRecord) !void {
        try self.ensureSchema(schema_sessions_kvbuf);

        // Extract tags from metadata_json for search
        const tags_text = try extractTagsFromMetadata(self.allocator, record.metadata_json);
        defer if (tags_text) |t| self.allocator.free(t);

        const payload = try encodeSessionRecordKvBuf(self.allocator, record, tags_text);
        defer self.allocator.free(payload);
        ...
        try self.fs_writer.appendRow(schema_sessions_kvbuf, &columns);
    }
    ...
    pub fn writeEmbedding(
        ...
        try self.ensureSchema(schema_embeddings);
        ...
        try self.fs_writer.appendRow(schema_embeddings, &columns);
    }

    fn ensureSchema(self: *TableAdapter, schema_id: u16) !void {
        if (self.fs_writer.schema_id) |current| {
            if (current != schema_id) {
                try self.fs_writer.flushBlock();
                self.fs_writer.resetSchema();
            }
        }
    }
    ...
    /// Opens a read-only adapter, scans with hash-based early exit, and verifies
    /// the session_id to protect against hash collisions.
    ///
    /// Returns the matched record. Caller owns the returned records slice and
    /// must free it with freeScannedSessionRecords().
...
pub fn forkSession(
    ...
    try owned.adapter.fs_writer.flushBlock();
}
...
test "TableAdapter.onEvent writes wal" {
    ...
    const wal_path = try std.fmt.allocPrint(std.testing.allocator, "chat/{s}", .{be.fs_writer.wal_name});
...
// =============================================================================
// Schema 5 (KvBuf) and Tags Integration Tests
// =============================================================================

test "scanSessionsFiltered search_query matches tags" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    // Write sessions with tags
    {
        var be = try TableAdapter.init(std.testing.allocator, root_path, "tags-test-1");
        defer be.backend().deinit();

        const session = SessionRecord{
            .session_id = "tags-test-1",
            .model = null,
            .title = "My Chat",
            .system_prompt = null,
            .config_json = null,
            ...
            .head_item_id = 0,
            .ttl_ts = 0,
            .metadata_json = "{\"tags\":[\"work\",\"project-x\"]}",
            .created_at_ms = 100,
            .updated_at_ms = 100,
        };
    ...
    {
        var be = try TableAdapter.init(std.testing.allocator, root_path, "tags-test-2");
        defer be.backend().deinit();

        const session = SessionRecord{
            .session_id = "tags-test-2",
            .model = null,
            .title = "Personal Chat",
            .system_prompt = null,
            .config_json = null,
            ...
            .head_item_id = 0,
            .ttl_ts = 0,
            .metadata_json = "{\"tags\":[\"personal\",\"home\"]}",
            .created_at_ms = 200,
            .updated_at_ms = 200,
        };
    ...
    const records = try adapter.scanSessionsFiltered(std.testing.allocator, .{ .search_query = "work" });
    defer freeScannedSessionRecords(std.testing.allocator, records);

    try std.testing.expectEqual(@as(usize, 1), records.len);
    try std.testing.expectEqualStrings("tags-test-1", records[0].session_id);
    // Verify tags_text was populated
    try std.testing.expectEqualStrings("work project-x", records[0].tags_text.?);
}

test "scanSessionsFiltered search_query matches partial tag" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    {
        var be = try TableAdapter.init(std.testing.allocator, root_path, "partial-tag-test");
        defer be.backend().deinit();

        const session = SessionRecord{
            .session_id = "partial-tag-test",
            .model = null,
            .title = "Chat",
            .system_prompt = null,
            .config_json = null,
            ...
            .head_item_id = 0,
            .ttl_ts = 0,
            .metadata_json = "{\"tags\":[\"project-alpha\",\"development\"]}",
            .created_at_ms = 100,
            .updated_at_ms = 100,
        };
    ...
    // Search for partial match "alpha"
    const records = try adapter.scanSessionsFiltered(std.testing.allocator, .{ .search_query = "alpha" });
    defer freeScannedSessionRecords(std.testing.allocator, records);

    try std.testing.expectEqual(@as(usize, 1), records.len);
    try std.testing.expectEqualStrings("partial-tag-test", records[0].session_id);
}

test "scanSessionsFiltered search_query case insensitive tags" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    {
        var be = try TableAdapter.init(std.testing.allocator, root_path, "case-test");
        defer be.backend().deinit();
        ...
        const session = SessionRecord{
            ...
            .head_item_id = 0,
            .ttl_ts = 0,
            .metadata_json = "{\"tags\":[\"WorkProject\"]}",
            .created_at_ms = 100,
            .updated_at_ms = 100,
        };
    ...
    const records = try adapter.scanSessionsFiltered(std.testing.allocator, .{ .search_query = "WORKPROJECT" });
    defer freeScannedSessionRecords(std.testing.allocator, records);

    try std.testing.expectEqual(@as(usize, 1), records.len);
}

test "scanSessionsFiltered tags_filter exact word match" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    // Write sessions with different tags
    {
        var be = try TableAdapter.init(std.testing.allocator, root_path, "tags-exact-1");
        defer be.backend().deinit();

        const session = SessionRecord{
            .session_id = "tags-exact-1",
            .model = null,
            .title = "Rust Project",
            .system_prompt = null,
            .config_json = null,
            ...
            .head_item_id = 0,
            .ttl_ts = 0,
            .metadata_json = "{\"tags\":[\"rust\",\"python\",\"work\"]}",
            .created_at_ms = 100,
            .updated_at_ms = 100,
        };
    ...
    {
        var be = try TableAdapter.init(std.testing.allocator, root_path, "tags-exact-2");
        defer be.backend().deinit();

        const session = SessionRecord{
            .session_id = "tags-exact-2",
            .model = null,
            .title = "Python Only",
            .system_prompt = null,
            .config_json = null,
            ...
            .head_item_id = 0,
            .ttl_ts = 0,
            .metadata_json = "{\"tags\":[\"python\",\"personal\"]}",
            .created_at_ms = 200,
            .updated_at_ms = 200,
        };
    ...
    {
        var be = try TableAdapter.init(std.testing.allocator, root_path, "tags-exact-3");
        defer be.backend().deinit();

        const session = SessionRecord{
            .session_id = "tags-exact-3",
            .model = null,
            .title = "Rusty Tools",
            .system_prompt = null,
            .config_json = null,
            ...
            .head_item_id = 0,
            .ttl_ts = 0,
            // "rusty" is NOT the same as "rust" - exact word match
            .metadata_json = "{\"tags\":[\"rusty\",\"tools\"]}",
            .created_at_ms = 300,
            .updated_at_ms = 300,
        };
    ...
    // tags_filter: exact match for "rust" - should NOT match "rusty"
    {
        const records = try adapter.scanSessionsFiltered(std.testing.allocator, .{ .tags_filter = "rust" });
        defer freeScannedSessionRecords(std.testing.allocator, records);

        try std.testing.expectEqual(@as(usize, 1), records.len);
        try std.testing.expectEqualStrings("tags-exact-1", records[0].session_id);
    }

    // tags_filter: AND logic - must have both "rust" AND "python"
    {
        const records = try adapter.scanSessionsFiltered(std.testing.allocator, .{ .tags_filter = "rust python" });
        defer freeScannedSessionRecords(std.testing.allocator, records);
    ...
    // tags_filter_any: OR logic - must have "rust" OR "personal"
    {
        const records = try adapter.scanSessionsFiltered(std.testing.allocator, .{ .tags_filter_any = "rust personal" });
        defer freeScannedSessionRecords(std.testing.allocator, records);

        try std.testing.expectEqual(@as(usize, 2), records.len);
        // Results are sorted by updated_at DESC, so tags-exact-2 comes first
    }

    // tags_filter_any: single tag
    {
        const records = try adapter.scanSessionsFiltered(std.testing.allocator, .{ .tags_filter_any = "tools" });
        defer freeScannedSessionRecords(std.testing.allocator, records);

        try std.testing.expectEqual(@as(usize, 1), records.len);
        try std.testing.expectEqualStrings("tags-exact-3", records[0].session_id);
    }

    // tags_filter: no match
    {
        const records = try adapter.scanSessionsFiltered(std.testing.allocator, .{ .tags_filter = "java" });
        defer freeScannedSessionRecords(std.testing.allocator, records);
