...
/// Check if a space-separated tags_text contains a specific tag (exact word match).
/// Case-insensitive ASCII comparison. Returns true if the tag appears as a whole word.
pub fn tagsContainWord(tags_text: []const u8, tag: []const u8) bool {
    if (tag.len == 0) return true;
    if (tags_text.len == 0) return false;

    var start: usize = 0;
    while (start < tags_text.len) {
        // Skip leading spaces
        while (start < tags_text.len and tags_text[start] == ' ') : (start += 1) {}
        if (start >= tags_text.len) break;

        // Find end of word
        var end = start;
        while (end < tags_text.len and tags_text[end] != ' ') : (end += 1) {}

        const word = tags_text[start..end];
        if (word.len == tag.len) {
            var match = true;
            for (0..tag.len) |i| {
                if (std.ascii.toLower(word[i]) != std.ascii.toLower(tag[i])) {
                    match = false;
                    break;
                }
            }
...
/// Check if tags_text contains ALL of the tags in filter (AND logic).
/// Both tags_text and filter are space-separated. Case-insensitive.
pub fn tagsMatchAll(tags_text: []const u8, filter: []const u8) bool {
    var start: usize = 0;
    while (start < filter.len) {
        ...
        // Find end of filter tag
        var end = start;
        while (end < filter.len and filter[end] != ' ') : (end += 1) {}

        const filter_tag = filter[start..end];
        if (filter_tag.len > 0 and !tagsContainWord(tags_text, filter_tag)) {
            return false; // Missing a required tag
        }
        start = end;
    }
    return true; // All filter tags found
}

/// Check if tags_text contains ANY of the tags in filter (OR logic).
/// Both tags_text and filter are space-separated. Case-insensitive.
pub fn tagsMatchAny(tags_text: []const u8, filter: []const u8) bool {
    var start: usize = 0;
    while (start < filter.len) {
        // Skip leading spaces
        while (start < filter.len and filter[start] == ' ') : (start += 1) {}
    ...
    return false; // No filter tags found
}
...
// ---------------------------------------------------------------------------
// tagsContainWord / tagsMatchAll / tagsMatchAny unit tests
// ---------------------------------------------------------------------------

test "tagsContainWord exact match" {
    try std.testing.expect(tagsContainWord("rust python go", "rust"));
    try std.testing.expect(tagsContainWord("rust python go", "python"));
    try std.testing.expect(tagsContainWord("rust python go", "go"));
}

test "tagsContainWord case insensitive" {
    try std.testing.expect(tagsContainWord("Rust Python Go", "rust"));
    try std.testing.expect(tagsContainWord("rust python go", "PYTHON"));
    try std.testing.expect(tagsContainWord("rust python go", "Go"));
}

test "tagsContainWord no partial match" {
    // "rust" should not match "rusty" (partial substring)
    try std.testing.expect(!tagsContainWord("rusty python", "rust"));
    // "py" should not match "python" (partial word)
    try std.testing.expect(!tagsContainWord("rust python go", "py"));
}

test "tagsContainWord no match" {
    try std.testing.expect(!tagsContainWord("rust python go", "java"));
    try std.testing.expect(!tagsContainWord("", "rust"));
}

test "tagsContainWord empty tag" {
    try std.testing.expect(tagsContainWord("rust python", ""));
    try std.testing.expect(tagsContainWord("", ""));
}

test "tagsContainWord multiple spaces" {
    try std.testing.expect(tagsContainWord("rust  python   go", "python"));
    try std.testing.expect(tagsContainWord("  rust  ", "rust"));
}

test "tagsMatchAll single tag" {
    try std.testing.expect(tagsMatchAll("rust python go", "rust"));
    try std.testing.expect(!tagsMatchAll("rust python go", "java"));
}

test "tagsMatchAll multiple tags" {
    try std.testing.expect(tagsMatchAll("rust python go", "rust python"));
    try std.testing.expect(tagsMatchAll("rust python go", "python go"));
    try std.testing.expect(tagsMatchAll("rust python go", "go rust"));
    try std.testing.expect(!tagsMatchAll("rust python go", "rust java"));
}

test "tagsMatchAll empty filter" {
    try std.testing.expect(tagsMatchAll("rust python go", ""));
    try std.testing.expect(tagsMatchAll("", ""));
}

test "tagsMatchAny single tag" {
    try std.testing.expect(tagsMatchAny("rust python go", "rust"));
    try std.testing.expect(!tagsMatchAny("rust python go", "java"));
}

test "tagsMatchAny multiple tags" {
    try std.testing.expect(tagsMatchAny("rust python go", "java rust"));
    try std.testing.expect(tagsMatchAny("rust python go", "java kotlin python"));
    try std.testing.expect(!tagsMatchAny("rust python go", "java kotlin"));
}

test "tagsMatchAny empty filter" {
    try std.testing.expect(!tagsMatchAny("rust python go", ""));
    try std.testing.expect(!tagsMatchAny("", ""));
}
