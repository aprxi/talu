...
test "CPU transport endpoint exposes host slices and target buffers without copying" {
    ...
    try std.testing.expectEqual(main.inference.bridge.TensorFramePayloadLocationHint{ .cpu = {} }, hint);

    const decode_source = try endpoint.hostDecodeActivationSlice(&backend, 0, 4);
    try std.testing.expectEqualSlices(u8, &.{ 1, 2, 3, 4 }, decode_source.bytes);
    const prefill_source = try endpoint.hostPrefillActivationSlice(&backend, 3);
    try std.testing.expectEqualSlices(u8, &.{ 9, 10, 11 }, prefill_source.bytes);

    const decode_target = try endpoint.decodeInputBuffer(&backend, 0, 2);
    decode_target.bytes[0] = 42;
    try std.testing.expectEqual(@as(u8, 42), backend.slot_bytes[0]);

    const prefill_target = try endpoint.prefillInputBuffer(&backend, 2);
