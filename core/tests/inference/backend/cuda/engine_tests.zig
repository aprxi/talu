...
const computeStageRuntimeSurface = CudaBackend.computeStageRuntimeSurface;
...
test "computeStageRuntimeSurface uses explicit ownership, not layer position" {
    const internal_owned_surface = computeStageRuntimeSurface(.{
        .layer_range = .{ .start = 5, .end = 10 },
        .owns_embedding = true,
        .owns_projection = true,
    });
    try std.testing.expect(!internal_owned_surface.skip_embedding);
    try std.testing.expect(!internal_owned_surface.skip_projection);

    const full_layer_only_surface = computeStageRuntimeSurface(.{
        .layer_range = .{ .start = 0, .end = 32 },
        .owns_embedding = false,
        .owns_projection = false,
    });
    try std.testing.expect(full_layer_only_surface.skip_embedding);
    try std.testing.expect(full_layer_only_surface.skip_projection);
}
