...
test "noop activation stage validates empty pipeline input" {
...
test "cuda activation stage exposes local pipeline transport methods" {
    var trace = MockCudaActivationTrace{};
    var backend = MockCudaActivationBackend{
        .device = .{ .trace = &trace },
        .compute_stream = 11,
...
test "cuda activation stage uploads segmented pipeline payloads" {
    var trace = MockCudaActivationTrace{};
    var backend = MockCudaActivationBackend{
        .device = .{ .trace = &trace },
        .runtime_buffers = .{ .input_dev = .{ .pointer = 100, .size = 16, .trace = &trace } },
